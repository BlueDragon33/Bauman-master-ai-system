...
- B10 chưa được load bởi learner runtime và không sửa content, subject engine,
  storage, offline manager hoặc Service Worker.
- Remote gate: feature commit
  `fa7eb674782444593f7035acfd70cba018495362`, run `32725455874`, context
  `migration/l6-b10-renderer-bridge` = `success`; B1-B9 contexts trong cùng run
  đều `success`.
...
11. Regression reference subjects + rollback checkpoint; chỉ PASS khi Russian/Math không giảm chức năng và một subject nhẹ có reviewed capability path dựng được bằng Factory, khép `LIGHT-POLICY-CAPABILITY-GAP` mà không hạ chuẩn B3.
    **PASS**: deterministic activation/rollback 39/39; protection mutation
    12/12; browser 21/21; 3/3 viewport, 3/3 explicit offline pack và 3/3
    Foundation/Math/Russian offline route. Kích hoạt duy nhất
    `foundation:f_s01_l1`; context `migration/l6-b11-reference-runtime`
    thành công tại run `32792582696`. L5 full runtime/offline/browser regression
    tiếp tục PASS tại run `32792704340`; `main` không thay đổi.
...
7. Master-ready evidence + spaced-review hooks cho Russian/Math/Foundation.
   **PASS**: catalog/read-only materializer phủ đúng 406 lesson gồm 26 Russian,
   365 Math và 15 Foundation; khai báo 2.030 evidence slot còn thiếu, 1.250
   review template chưa lên lịch và 812 retention window bắt buộc. Chỉ được
   neo lịch bằng UTC sau khi các stage trước retention đã verified; attempt sớm,
   prompt lặp/lộ đáp án, score/source sai hoặc verifier không đủ quyền đều fail
   closed. AI/self/peer chỉ advisory; review đạt chỉ tạo verification candidate,
   không ghi learner state hay tự gắn Master-ready. Gate 83/83 cùng 10/10
   protection mutation PASS; catalog SHA-256

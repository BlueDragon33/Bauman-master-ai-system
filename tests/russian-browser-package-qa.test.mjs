...
{const x=copy();x.browserCapabilities.playbackCompletionHookRequired=false;assert.throws(()=>validateContract(x),/Playback evidence contract weakened/)}
{const x=copy();x.browserCapabilities.interruptedPlaybackMustNotCountListening=false;assert.throws(()=>validateContract(x),/Playback evidence contract weakened/)}
...
assert.throws(()=>validateBrowserCapabilityBehavior(bundle.capability.replaceAll('if(token!==activeSpeechToken)return;','')),/Interrupted playback emitted completion evidence/);
assert.throws(()=>validateBrowserCapability({...bundle,core:bundle.core.replace("onEnd:meta=>{if(inPracticeMode()){markPracticeLineHeard(d,idx);render()}","onEnd:meta=>{if(inPracticeMode()){render()}")}),/playback-completion confirmed/);
...
console.log(JSON.stringify({negativeCases:17},null,2));

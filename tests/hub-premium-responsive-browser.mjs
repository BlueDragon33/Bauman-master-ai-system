...
try{
  ...
  for(const [label,width,height,mode] of cases){
    ...
    const snap=await page.evaluate(()=>{const appearance=document.getElementById('appearanceBtn'),topbar=document.querySelector('.topbar'),after=appearance?getComputedStyle(appearance,'::after').content:'';return{mode:document.body.dataset.hubViewport,client:document.documentElement.clientWidth,scroll:document.documentElement.scrollWidth,hero:!!document.querySelector('.hub2-hero'),assistant:!!document.querySelector('.hub2-assistant'),appearance:!!appearance,appearanceAfter:after,search:!!document.getElementById('hubGlobalSearch'),topbarRight:topbar?.getBoundingClientRect().right||0}});
    assert.equal(snap.mode,mode,`${label}: viewport classification drift`);assert.ok(snap.scroll<=snap.client+2,`${label}: horizontal overflow ${snap.scroll}/${snap.client}`);assert.ok(snap.hero&&snap.assistant&&snap.appearance&&snap.search,`${label}: primary Hub controls missing`);assert.ok(snap.topbarRight<=snap.client+1,`${label}: topbar outside viewport`);if(mode==='phone')assert.match(snap.appearanceAfter,/Giao diện/,`${label}: Giao diện label is not visible`);

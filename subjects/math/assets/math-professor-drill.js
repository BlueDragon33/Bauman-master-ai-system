 ...
 * Source priority: companion content -> accepted lesson assessment -> semantic theory slides.
...
(function mathProfessorDrill(global){
  ...
  function chapterId(){return String(state().e186Path?.chapterId||state().e169Path?.chapterId||state().e129ChapterId||'')}
  ...
  async function fetchJson(path){try{const r=await fetch(path,{cache:'no-store'});if(!r.ok)throw new Error(`${path} HTTP ${r.status}`);const data=await r.json();return{ok:true,data,records:recordsOf(data),path}}catch(error){return{ok:false,data:null,records:[],path,error:String(error?.message||error)}}}
  async function loadSources(){
    if(sources)return sources;
    const [professor,questions,theory]=await Promise.all([
      fetchJson('data/professor_qa_content.json'),
      fetchJson('data/question_bank_content.json'),
      fetchJson(THEORY_SOURCE)
    ]);
    sources={professor,questions,theory};return sources;
  }
  async function loadAssessment(id=lessonId()){
    const path=ASSESSMENT_SOURCES[id];if(!path)return null;
    if(!assessmentCache[path])assessmentCache[path]=await fetchJson(path);
    return assessmentCache[path];
  }
  function theoryRecords(){
    const raw=global.DB?.theory_lecture_content,db=recordsOf(raw);
    return db.length?db:recordsOf(sources?.theory?.data);
  }
  ...
  function assessmentItems(){
    const id=lessonId(),path=ASSESSMENT_SOURCES[id],src=path&&assessmentCache[path];
    const data=src?.ok?src.data:null;if(!data||String(data.lessonId||'')!==id)return[];
    const rows=Array.isArray(data.professorQA)?data.professorQA:[];
    return rows.map((row,i)=>{
      const question=text(row.prompt||row.question||row.title);if(!question)return null;
      const evidence=Array.isArray(row.expectedEvidence)?row.expectedEvidence.map(text).filter(Boolean):[];
      const answer=text(row.answer||row.expectedAnswer||row.solution)||evidence.join(' · ');
      return{key:`assessment:${id}:${row.id||i}`,source:'accepted assessment',title:`${row.id||`PQ${i+1}`} · Professor Q&A`,question,answer,slideIndex:null,role:'professor_qa'};
    }).filter(Boolean);
  }
  function build(){
    const companion=companionItems(),assessment=assessmentItems(),semantic=semanticItems();
    items=companion.length?companion:(assessment.length?assessment:semantic);
    if(index>=items.length)index=Math.max(0,items.length-1);return items;
  }
  function ensure(){
  ...
  function render(){
    ...
    if(!items.length){body.innerHTML='<div class="math-pd-empty">Bài hiện tại chưa có professor/question record, accepted assessment hoặc slide semantic phù hợp. Hệ thống không tự sinh câu hỏi thay thế.</div>';if(progress)progress.textContent='0 câu hỏi';return}
  ...
  async function open(){ensure();await loadSources();await loadAssessment();index=0;render();$('#mathProfessorDrill')?.classList.add('open')}
  ...
  function bind(){document.addEventListener('click',e=>{const action=e.target.closest('[data-pd]')?.dataset.pd;if(action){e.preventDefault();if(action==='open')open();if(action==='close')close();if(action==='prev'){index=Math.max(0,index-1);render()}if(action==='next'){index=Math.min(Math.max(0,items.length-1),index+1);render()}if(action==='reveal')$('#mathPdAnswer')?.classList.toggle('open');return}const conf=e.target.closest('[data-pd-confidence]')?.dataset.pdConfidence,key=e.target.closest('[data-pd-key]')?.dataset.pdKey;if(conf&&key){e.preventDefault();writeConfidence(key,conf);return}const slide=e.target.closest('[data-pd-slide]')?.dataset.pdSlide;if(slide!=null){e.preventDefault();openSlide(Number(slide)||0);return}if(e.target.closest('[data-e186-pick],[data-e129-lesson],[data-lf-step],[data-math-nav]'))setTimeout(ensureInlineButton,180)},true);document.addEventListener('keydown',e=>{if(e.key==='Escape')close();if($('#mathProfessorDrill')?.classList.contains('open')&&e.altKey&&e.key==='ArrowRight'){e.preventDefault();index=Math.min(Math.max(0,items.length-1),index+1);render()}if($('#mathProfessorDrill')?.classList.contains('open')&&e.altKey&&e.key==='ArrowLeft'){e.preventDefault();index=Math.max(0,index-1);render()}})}
  function sourceName(){if(companionItems().length)return'companion';if(assessmentItems().length)return'accepted-assessment';return'theory-semantic-fallback'}
  function selfCheck(){build();return{release:RELEASE,ready:!!$('#mathProfessorDrill'),lessonId:lessonId()||null,items:items.length,source:sourceName(),assessmentPath:ASSESSMENT_SOURCES[lessonId()]||null,sampleRecordsRendered:false,generatedQuestions:false,gradingAuthority:false,localConfidenceOnly:true,academicWrites:false,mutationObserver:false}}
  function init(){if(!document.body||document.body.dataset.mathProfessorDrill==='1')return;document.body.dataset.mathProfessorDrill='1';ensure();bind();loadSources().then(()=>loadAssessment()).then(()=>{[700,1600,3000].forEach(ms=>setTimeout(()=>{ensureInlineButton();build()},ms))});global.BAUMAN_MATH_PROFESSOR_DRILL={release:RELEASE,open,close,refresh:async()=>{ensureInlineButton();await loadSources();await loadAssessment();build()},selfCheck}}

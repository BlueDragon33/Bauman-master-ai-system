...
(function(){
  ...
  function boot(){ensureStyle();registerOptionalSources();schedule();try{new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});}catch(_){};document.addEventListener('click',function(e){if(e.target&&e.target.closest('[data-e241-reference]')){e.preventDefault();e.stopPropagation();showReference();return;}if(e.target&&e.target.closest('[data-e241-full]')){e.preventDefault();e.stopPropagation();showFullView();}},true);window.addEventListener('keydown',function(e){if(e.key==='Escape'&&modal&&!modal.classList.contains('hidden')){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();closeModal();}},true);}

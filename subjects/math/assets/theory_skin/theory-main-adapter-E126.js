...
(function(){
  ...
  var learnModes=[['theory','📘','Lý thuyết','đang mở'],['exercises','📝','Bài tập','luyện'],['review','🔁','Ôn tập','vá lỗi'],['exam','🧪','Kiểm tra','20 câu']];
  function learnRail(active){return '<nav class="e126-learn-rail" aria-label="Bốn phần học tập">'+learnModes.map(function(m){return '<button class="'+(active===m[0]?'active':'')+'" data-e126-learn="'+esc(m[0])+'"><i>'+esc(m[1])+'</i><b>'+esc(m[2])+'</b><small>'+esc(m[3])+'</small></button>';}).join('')+'</nav>';}
  function render(){
    var xs=lessons();
    ...
    html=html.replace('<div class="e126-slide-rail">',learnRail('theory')+'<div class="e126-slide-rail">');
  ...
  document.addEventListener('click',function(e){
    ...
    var xs=lessons(), s=st();
    if(t.hasAttribute('data-e126-learn')){var mode=String(t.getAttribute('data-e126-learn')||'theory'); if(!/^(theory|exercises|review|exam)$/.test(mode))mode='theory'; s.view='learning'; s.learnTab=mode; s.e122Focus=mode; rerender(); e.preventDefault(); if(e.stopImmediatePropagation)e.stopImmediatePropagation(); else e.stopPropagation(); return;}

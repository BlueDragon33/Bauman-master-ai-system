...
(function mathRegressionGate(global){
  ...
  function moduleCheck(name,api){if(!api)return row(`module-${name}`,name,'fail','Global API chưa được nạp.');try{const r=api.selfCheck?.();const bad=r&&((r.academicWrites===true)||(r.newRouteEngine===true)||(r.routeEngineReplacement===true)||(r.mutationObserver===true));return row(`module-${name}`,name,bad?'fail':'pass',r?Object.entries(r).filter(([k,v])=>['release','ready','loaded','academicWrites','mutationObserver','newRouteEngine','routeEngineReplacement'].includes(k)).map(([k,v])=>`${k}=${v}`).join(' · '):'API có mặt') }catch(e){return row(`module-${name}`,name,'warn',String(e?.message||e))}}
  ...
  function init(){if(!document.body||document.body.dataset.mathRegressionGate==='1')return;document.body.dataset.mathRegressionGate='1';ensure();bind();[1000,2600].forEach(ms=>setTimeout(run,ms));global.BAUMAN_MATH_REGRESSION_GATE={release:RELEASE,run,open,close,getReport:()=>report,selfCheck}}

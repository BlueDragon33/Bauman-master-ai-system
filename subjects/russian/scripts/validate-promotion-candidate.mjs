...
const jsOrder=['assets/subject-adapter.js','assets/content-contract.js','../shared/host-bridge.js','assets/planning-bridge.js','assets/russian-optional-data-loader.js','assets/core.js','assets/learning-state.js','assets/learning-flow.js','assets/vocab-srs.js','assets/speaking-coach.js','assets/academic-language.js','assets/ai-mentor-guard.js','assets/runtime-optimizer.js','assets/russian-reference-ui.js'];
...
for(const item of [...cssOrder.filter(x=>!['assets/core.css'].includes(x)),...jsOrder.filter(x=>x.startsWith('assets/'))])need(sw,`./${item}`,`Service worker shell missing ${item}`);
...
const runtimeJs=['assets/core.js','assets/learning-state.js','assets/learning-flow.js','assets/vocab-srs.js','assets/speaking-coach.js','assets/academic-language.js','assets/ai-mentor-guard.js','assets/runtime-optimizer.js','assets/russian-reference-ui.js'].map(read).join('\n');
...
console.log('Checks: no runtime backups, stable additive load order, canonical resume/review/SRS/academic storage preserved, AI read-only, Russian-scoped offline shell, responsive runtime status, no destructive storage reset.');

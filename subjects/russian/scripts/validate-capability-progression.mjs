...
need(cap,"w.withSnapshot<band.writingNeed?'writing'",'Capability band must surface missing writing evidence');
need(cap,"w.rewrites<band.rewriteNeed?'rewrite'",'Capability band must surface missing rewrite evidence');
need(cap,"if(step==='writing'||step==='rewrite')return {view:'writing'",'Writing gaps must route to the writing surface');

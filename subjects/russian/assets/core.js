...
function notifyListeningPlayback(d,i,kind,meta={}){try{window.dispatchEvent(new CustomEvent('russian:listening-playback-started',{detail:{key:practiceHeardKey(d,i),dialogueId:str(d?.id||d?.title||'dialogue'),lineIndex:Number(i)||0,kind,lang:meta?.lang||'ru-RU',voiceName:meta?.voiceName||'',verifiedRussianVoice:Boolean(meta?.verifiedRussianVoice)}}))}catch(_){}}
...
function handleClick(e){
 ...
 if(act==='speak-line'){const d=currentDialogue(),idx=activeLineIndex(); const line=dialogueTurns(d)[idx]||{}; const played=speak(line.ru||line.text||line.text_ru||line,.85,{onStart:meta=>{if(inPracticeMode()){markPracticeLineHeard(d,idx);render()}notifyListeningPlayback(d,idx,'normal',meta)}}); if(!played)toast('Chưa phát được âm tiếng Nga; lượt nghe không được tính.')}
 if(act==='speak-line-slow'){const d=currentDialogue(), idx=activeLineIndex(); if(inPracticeMode()&&practiceLineHeardCount(d,idx)<2){toast('Nghe tốc độ thường 2 lượt trước khi dùng nghe chậm');return} const line=dialogueTurns(d)[idx]||{}; const played=speak(line.ru||line.text||line.text_ru||line,.62,{onStart:meta=>notifyListeningPlayback(d,idx,'slow',meta)}); if(!played)toast('Chưa phát được âm chậm; lượt sửa nghe không được tính.')}

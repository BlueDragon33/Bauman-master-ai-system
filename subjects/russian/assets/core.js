...
function speak(text,rate=.85){ const cap=window.RussianBrowserCapabilities; if(cap?.speak)return cap.speak(text,rate); if(!text||!('speechSynthesis' in window)||!window.SpeechSynthesisUtterance){toast('Trình duyệt không hỗ trợ phát âm tiếng Nga');return false;} const u=new SpeechSynthesisUtterance(text); u.lang=A.speech?.lang||'ru-RU'; u.rate=rate; speechSynthesis.cancel(); speechSynthesis.speak(u); return true; }

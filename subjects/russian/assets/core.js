...
function notifyListeningPlaybackCompleted(d,i,kind,meta={}){try{window.dispatchEvent(new CustomEvent('russian:listening-playback-completed',{detail:{key:practiceHeardKey(d,i),dialogueId:str(d?.id||d?.title||'dialogue'),lineIndex:Number(i)||0,kind,lang:meta?.lang||'ru-RU',voiceName:meta?.voiceName||'',verifiedRussianVoice:Boolean(meta?.verifiedRussianVoice)}}))}catch(_){}}
...
function notifySpeakingRecordingStarted(d,i,lessonId=state.lessonId){try{window.dispatchEvent(new CustomEvent('russian:speaking-recording-started',{detail:{lessonId:str(lessonId||''),dialogueId:str(d?.id||d?.title||'dialogue'),lineIndex:Number(i)||0,key:dialogueLineKey(d,i),lang:'ru-RU'}}))}catch(_){}}
function notifySpeakingRecordingResult(d,i,transcript,score,lessonId=state.lessonId){try{window.dispatchEvent(new CustomEvent('russian:speaking-recording-result',{detail:{lessonId:str(lessonId||''),dialogueId:str(d?.id||d?.title||'dialogue'),lineIndex:Number(i)||0,key:dialogueLineKey(d,i),lang:'ru-RU',transcript:str(transcript),score:Number(score)||0,ok:Number(score)>=70}}))}catch(_){}}
function startLineRecording(){
 ...
 const resultStoreKey=inPracticeMode()?'practiceSpeechResults':'dialogueSpeechResults', dialogueIdAtStart=str(d?.id||d?.title||'dialogue'), lessonIdAtStart=str(state.lessonId||'');
 const target=dialogueText(turns[idx]||{}); if(!target){toast('Chưa có câu để luyện');return false}
 if(!SR){toast('Trình duyệt chưa hỗ trợ nhận diện giọng nói. Hãy dùng Chrome/Edge trên Live Server.');return false}
 const token=++speechRecognitionToken;
 try{if(speechRecognizer)speechRecognizer.stop()}catch(_){ }
 const rec=new SR(); speechRecognizer=rec; let resultConsumed=false; rec.lang='ru-RU'; rec.interimResults=false; rec.maxAlternatives=1; rec.continuous=false;
 rec.onstart=()=>{if(token!==speechRecognitionToken)return;state.speechRecording=true;save();render();notifySpeakingRecordingStarted(d,idx,lessonIdAtStart);toast('Đang nghe tiếng Nga của bạn...')};
 rec.onresult=ev=>{if(token!==speechRecognitionToken||resultConsumed)return;resultConsumed=true;const transcript=str(ev.results?.[0]?.[0]?.transcript||'').trim(); if(!transcript){state.speechRecording=false;save();render();toast('Không nhận được câu nói tiếng Nga; attempt chưa được tính.');return} const score=Math.round(speechSimilarity(transcript,target)*100); state[resultStoreKey]=state[resultStoreKey]||{}; const store=state[resultStoreKey]; store[dialogueLineKey(d,idx)]={score,transcript,target,at:Date.now(),ok:score>=70}; notifySpeakingRecordingResult(d,idx,transcript,score,lessonIdAtStart); state.speechRecording=false; save(); render(); toast(score>=70?'Đạt câu này, có thể qua câu tiếp':'Chưa đạt, luyện lại câu này'); if(score>=70&&state.speechAutoNext)setTimeout(()=>{const sameSurface=(resultStoreKey==='practiceSpeechResults'&&inPracticeMode())||(resultStoreKey==='dialogueSpeechResults'&&!inPracticeMode()); if(token===speechRecognitionToken&&sameSurface&&currentDialogueId()===dialogueIdAtStart&&activeLineIndex()===idx)moveDialogueLine(1)},520)};
 rec.onerror=()=>{if(token!==speechRecognitionToken||resultConsumed)return;resultConsumed=true;state.speechRecording=false; save(); render(); toast('Không nghe rõ. Kiểm tra micro rồi thử lại')};
 rec.onend=()=>{if(token!==speechRecognitionToken)return;resultConsumed=true;if(state.speechRecording){state.speechRecording=false; save(); render();}}
...
function startDeepSpeakingRecording(){
 const SR=window.SpeechRecognition||window.webkitSpeechRecognition;
 const unit=currentDeepUnit(currentDialogue()), id=deepUnitKey(unit), mode=state.deepSpeakingMode||'overview';
 if(!unit||!id||mode==='overview'){toast('Chọn một chế độ Deep Speaking trước khi ghi âm');return false}
 if(!SR){toast('Trình duyệt chưa hỗ trợ nhận diện giọng nói. Hãy dùng Chrome/Edge trên Live Server.');return false}
 const token=++speechRecognitionToken;
 try{if(speechRecognizer)speechRecognizer.stop()}catch(_){ }
 const rec=new SR(); speechRecognizer=rec; let resultConsumed=false; rec.lang='ru-RU'; rec.interimResults=false; rec.maxAlternatives=1; rec.continuous=false;
 rec.onstart=()=>{if(token!==speechRecognitionToken)return;state.speechRecording=true;save();render();toast('Đang ghi Deep Speaking...')};
 rec.onresult=ev=>{if(token!==speechRecognitionToken||resultConsumed)return;resultConsumed=true;const transcript=str(ev.results?.[0]?.[0]?.transcript||'').trim(); if(!transcript){state.speechRecording=false;save();render();toast('Không nhận được câu nói tiếng Nga; Deep Speaking attempt chưa được tính.');return} const p=state.deepSpeakingProgress={done:{},weak:{},attempts:{},lastMode:{},lastAttemptAt:{},...(state.deepSpeakingProgress||{})}; p.attempts={...(p.attempts||{})}; p.lastMode={...(p.lastMode||{})}; p.lastAttemptAt={...(p.lastAttemptAt||{})}; p.attempts[id]=Number(p.attempts[id]||0)+1; p.lastMode[id]=mode; p.lastAttemptAt[id]=Date.now(); state.speechRecording=false; save(); render(); try{window.dispatchEvent(new CustomEvent('russian:deep-speaking-recording-result',{detail:{unitId:id,mode,lang:'ru-RU',transcript}}))}catch(_){} toast('Đã ghi nhận một Deep Speaking attempt từ lời nói thực.')};
 rec.onerror=()=>{if(token!==speechRecognitionToken||resultConsumed)return;resultConsumed=true;state.speechRecording=false;save();render();toast('Không nhận được Deep Speaking. Kiểm tra micro rồi thử lại')};
 rec.onend=()=>{if(token!==speechRecognitionToken)return;resultConsumed=true;if(state.speechRecording){state.speechRecording=false;save();render();}}
 try{rec.start();toast('Đang mở micro Deep Speaking...');return true}catch(_){state.speechRecording=false;save();render();toast('Micro chưa sẵn sàng');return false}
}
...
function renderDialogueSetupModal(){
 ...
 return `<div class="modal-body dialogue-setup-modal round2-dialogue-setup">
  ...
  <section class="dialogue-setup-grid">
    ...
    <label>Chủ đề/cuộc nói<select class="input" id="dsDialogue">${filtered.slice(0,180).map(d=>{const id=d.id||d.title; return `<option value="${esc(id)}" ${(active&&(active.id||active.title)===id)?'selected':''}>${esc(dialogueDirectTitle(d)||id)}</option>`}).join('')}</select></label>
...
function renderDeepMode(unit,mode){
 ...
 if(mode==='rapid'){const xs=deepLines(unit.rapid_drills||unit.core_phrases).slice(0,24); const line=xs[step%Math.max(1,xs.length)]||'Нет данных.'; return `<section class="panel deep-drill-card"><span class="chip">Rapid drill</span><h3>${esc(line)}</h3><p>Nghe, nói lại ngay, rồi bấm câu tiếp. Mục tiêu là bật câu tự nhiên.</p><div class="lesson-tools"><button class="btn" data-act="deep-prev">← Trước</button><button class="btn green" data-deep-speak="${esc(line)}">🔊 Nghe</button><button class="btn blue" data-act="deep-record">🎙️ Nói</button><button class="btn primary" data-act="deep-next">Câu tiếp →</button></div></section>`;}
 if(mode==='substitution'){const xs=deepLines(unit.substitution_drills||unit.substitution_slots).slice(0,24); return `<section class="panel deep-drill-card"><span class="chip">Substitution</span><h3>Thay mẫu câu</h3><ol>${xs.slice(step,step+8).map(x=>`<li>${esc(x)}</li>`).join('')||'<li>Chưa có mẫu thay thế.</li>'}</ol><div class="lesson-tools"><button class="btn" data-act="deep-prev">←</button><button class="btn blue" data-act="deep-record">🎙️ Nói</button><button class="btn primary" data-act="deep-next">Tiếp →</button></div></section>`;}
 if(mode==='shadowing'){const xs=deepLines(unit.shadowing_chain||unit.shadowing_sentences).slice(0,18); const line=xs[step%Math.max(1,xs.length)]||'Нет данных.'; return `<section class="panel deep-drill-card"><span class="chip">Shadowing</span><h3>${esc(line)}</h3><p>Nghe chậm, nói đè theo nhịp, sau đó nói lại không nhìn chữ.</p><div class="lesson-tools"><button class="btn" data-deep-speak="${esc(line)}">🔊 Thường</button><button class="btn blue" data-act="deep-record">🎙️ Nói</button><button class="btn" data-act="deep-prev">←</button><button class="btn primary" data-act="deep-next">Tiếp →</button></div></section>`;}
 if(mode==='monologue'){const m=unit.model_monologue||unit.monologue||{}; const title=m.prompt_ru||unit.monologue_prompt?.prompt_ru||'Монолог 45–90 секунд'; const body=m.ru||m.text_ru||m.answer_ru||deepLines(m).join('\n'); return `<section class="panel deep-drill-card"><span class="chip">Monologue</span><h3>${esc(title)}</h3><pre class="deep-ru-block">${esc(body||'Подготовьте короткий монолог по этой ситуации.')}</pre><div class="lesson-tools"><button class="btn" data-deep-speak="${esc(body||'')}">🔊 Nghe mẫu</button><button class="btn blue" data-act="deep-record">🎙️ Nói</button><button class="btn green" data-act="deep-mark-ok">Tự đánh giá: ổn</button><button class="btn warn" data-act="deep-mark-weak">Cần ôn lại</button></div></section>`;}
 if(mode==='qa'){const pairs=arr(unit.qa_pairs||unit.questions||[]); const p=pairs[step%Math.max(1,pairs.length)]||{}; const q=p.question_ru||p.q_ru||p.ru||'Как вы ответите?'; const a=p.answer_ru||p.a_ru||''; return `<section class="panel deep-drill-card"><span class="chip">Q&A áp lực</span><h3>${esc(q)}</h3>${a?`<details><summary>Xem gợi ý trả lời</summary><p>${esc(a)}</p></details>`:''}<div class="lesson-tools"><button class="btn" data-deep-speak="${esc(q)}">🔊 Câu hỏi</button><button class="btn blue" data-act="deep-record">🎙️ Trả lời</button><button class="btn" data-act="deep-prev">←</button><button class="btn primary" data-act="deep-next">Câu tiếp →</button><button class="btn warn" data-act="deep-mark-weak">Cần ôn</button></div></section>`;}
 const phrases=deepLines(unit.core_phrases).slice(0,8); return `<section class="panel deep-drill-card"><span class="chip">Tổng quan luyện sâu</span><h3>${esc(deepUnitTitle(unit))}</h3><p>${esc(unit.scenario_ru||'')}</p><div class="deep-core-grid">${phrases.map(x=>`<span>${esc(x)}</span>`).join('')||'<span>Chọn chế độ drill ở trên.</span>'}</div></section>`;
}
...
function renderGrammar(){
 ...
 return `<div class="grammar-luxe-shell">
   ...
   <main class="panel grammar-main-card" data-grammar-pattern-id="${esc(g.id||'')}"><header class="grammar-main-head"><div><span class="chip">${esc(g.level||'A1')} · ${esc(g.track||'Ngữ pháp')}</span><h3>${esc(g.title||'Mục ngữ pháp')}</h3><p>${esc(g.why||'Học mục này để nối quy tắc với câu nói, bài viết và tình huống thật.')}</p></div><b class="grammar-index-pill">${xs.length?state.grammarIndex+1:0}/${xs.length}</b></header>
...
function storageSkeleton(source){
 ...
 const map={
  ...
  simulations:{id,stage,group:'custom',type:'dialogue_lab',title:'Mô phỏng mới',purpose:'Luyện phản xạ',turns:['']},
...
function aiContext(){
 ...
 return {stage:stageTitle(),goal:todayMissionText(),view:state.view,learnTab:state.learnTab,lessonTitle:A.lessonTitle?.(lesson)||lesson?.title||'',lessonSummary:A.lessonSubtitle?.(lesson)||lesson?.summary||'',vocab:vocabInfo(vocab),dialogueTitle:dialogueDirectTitle(dialogue)||'',dialoguePurpose:A.dialogueSubtitle?.(dialogue)||dialogue?.purpose_ru||dialogue?.context_title_ru||'',writingTitle:A.writingTitle?.(writing)||writing?.title||''};
}
...
function speak(text,rate=.85,hooks={}){ const cap=window.RussianBrowserCapabilities; if(cap?.speak)return cap.speak(text,rate,hooks); if(!text||!('speechSynthesis' in window)||!window.SpeechSynthesisUtterance){toast('Trình duyệt không hỗ trợ phát âm tiếng Nga');return false;} try{const u=new SpeechSynthesisUtterance(text); u.lang=A.speech?.lang||'ru-RU'; u.rate=Math.max(.45,Math.min(1.15,Number(rate)||.85)); u.onstart=event=>hooks?.onStart?.({event,lang:u.lang,voiceName:'',verifiedRussianVoice:false}); u.onend=event=>hooks?.onEnd?.({event,lang:u.lang,voiceName:''}); u.onerror=event=>hooks?.onError?.({event,lang:u.lang,voiceName:''}); speechSynthesis.cancel(); speechSynthesis.speak(u); return true;}catch(error){hooks?.onError?.({error,lang:'ru-RU',voiceName:''});toast('Không phát được âm tiếng Nga');return false;} }
...
function handleClick(e){
 ...
 if(act==='speak-line'){const d=currentDialogue(),idx=activeLineIndex(); const line=dialogueTurns(d)[idx]||{}; const played=speak(line.ru||line.text||line.text_ru||line,.85,{onEnd:meta=>{if(inPracticeMode()){markPracticeLineHeard(d,idx);render()}notifyListeningPlaybackCompleted(d,idx,'normal',meta)}}); if(!played)toast('Chưa phát được âm tiếng Nga; lượt nghe không được tính.')}
 if(act==='speak-line-slow'){const d=currentDialogue(), idx=activeLineIndex(); if(inPracticeMode()&&practiceLineHeardCount(d,idx)<2){toast('Nghe tốc độ thường 2 lượt trước khi dùng nghe chậm');return} const line=dialogueTurns(d)[idx]||{}; const played=speak(line.ru||line.text||line.text_ru||line,.62,{onEnd:meta=>notifyListeningPlaybackCompleted(d,idx,'slow',meta)}); if(!played)toast('Chưa phát được âm chậm; lượt sửa nghe không được tính.')}
 if(act==='speak-dialogue'){speak(speakDialogue(currentDialogue()),.82)}
 if(act==='record-line')startLineRecording();
 if(act==='mark-line-ok'){const d=currentDialogue(), idx=activeLineIndex(); if(inPracticeMode()&&!practiceLineHeard(d,idx)){toast('Hãy nghe mẫu trước khi tự đánh giá câu nói');return} try{window.dispatchEvent(new CustomEvent('russian:speaking-self-assessed',{detail:{lessonId:str(state.lessonId||''),dialogueId:str(d?.id||d?.title||'dialogue'),lineIndex:Number(idx)||0,key:dialogueLineKey(d,idx),rating:'ok'}}))}catch(_){} toast('Đã ghi tự đánh giá · không tính là bằng chứng nói.');}
 ...
 if(act==='deep-prev'){state.deepSpeakingStep=Math.max(0,(Number(state.deepSpeakingStep)||0)-1);save();render();return}
  if(act==='deep-record'){startDeepSpeakingRecording();return}
 if(act==='deep-mark-ok'){const unit=currentDeepUnit(currentDialogue()); if(unit){const id=deepUnitKey(unit), weakAt=Number(state.deepSpeakingProgress?.weak?.[id]||0), attemptAt=Number(state.deepSpeakingProgress?.lastAttemptAt?.[id]||0); if(weakAt&&attemptAt<=weakAt){toast('Hãy ghi âm ít nhất một lượt mới trước khi gỡ mục Cần ôn.');return} state.deepSpeakingProgress.done[id]=Date.now(); if(weakAt&&attemptAt>weakAt)delete state.deepSpeakingProgress.weak[id]; save();render();toast(weakAt?'Đã gỡ Cần ôn sau một lượt nói mới.':'Đã lưu tự đánh giá Deep Speaking; không thay thế bằng chứng nhận diện giọng nói.');}return}

...
function startDeepSpeakingRecording(){
 ...
 rec.onerror=()=>{if(token!==speechRecognitionToken)return;state.speechRecording=false;save();render();toast('Không nhận được Deep Speaking. Kiểm tra micro rồi thử lại')};
 rec.onend=()=>{if(token!==speechRecognitionToken)return;if(state.speechRecording){state.speechRecording=false;save();render();}}

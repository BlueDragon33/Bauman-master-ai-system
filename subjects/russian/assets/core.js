...
function startDeepSpeakingRecording(){
 ...
 rec.onresult=ev=>{if(token!==speechRecognitionToken||resultConsumed)return;resultConsumed=true;const transcript=str(ev.results?.[0]?.[0]?.transcript||'').trim(); if(!transcript){state.speechRecording=false;save();render();toast('Không nhận được câu nói tiếng Nga; Deep Speaking attempt chưa được tính.');return} const p=state.deepSpeakingProgress={done:{},weak:{},attempts:{},lastMode:{},lastAttemptAt:{},...(state.deepSpeakingProgress||{})}; p.attempts={...(p.attempts||{})}; p.lastMode={...(p.lastMode||{})}; p.attempts[id]=Number(p.attempts[id]||0)+1; p.lastMode[id]=mode; state.speechRecording=false; save(); render(); try{window.dispatchEvent(new CustomEvent('russian:deep-speaking-recording-result',{detail:{unitId:id,mode,lang:'ru-RU',transcript}}))}catch(_){} toast('Đã ghi nhận một Deep Speaking attempt từ lời nói thực.')};

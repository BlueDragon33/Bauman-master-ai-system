...
const VERSION='Math Survival Master V13.33 · Route Grammar Mind Check';
...
const SUBJECT_STORAGE=window.BaumanSubjectStorage.forSubject(A.id||'programming');
...
function getPracticeDialogues(){
 const ctx=activeLessonContext();
 const appBase=Array.isArray(DB['dialogue-bauman-az'])?DB['dialogue-bauman-az']:[];
 const base=byStage(appBase).filter(x=>ctx.id&&sameLesson(x,ctx.id));
 const groups=uniq(base.map(x=>A.dialogueGroup?.(x)||x.group||x.applicationDomain||'ứng dụng'));
 const diffs=uniq(base.map(x=>A.dialogueDifficulty?.(x)||x.difficulty||x.level||'applied'));
 ...
 let xs=base;
 if(state.practiceGroup!=='all')xs=xs.filter(x=>(A.dialogueGroup?.(x)||x.group||x.applicationDomain||'ứng dụng')===state.practiceGroup);
 if(state.practiceDifficulty!=='all')xs=xs.filter(x=>(A.dialogueDifficulty?.(x)||x.difficulty||x.level||'applied')===state.practiceDifficulty);
...
function unlockStagePart(){
 ...
 if(nextStage){state.stage=nextStage; state.stageGate=stageGateDefaults(nextStage); state.examGateSource=null; resetExamProgress(); save(); render(); toast('Đã mở khóa giai đoạn tiếp theo'); return;}
...
function buildShell(){ applyInterface(); $('#subjectLogo').textContent=A.ui?.logo||'Я'; $('#subjectTitle').textContent=A.ui?.title||'Programming Bauman'; $('#subjectSubtitle').textContent=A.ui?.subtitle||''; $('#coreLabel').textContent=A.ui?.coreLabel||'V12.82 LEARNING INTEGRITY FINAL'; $('#stageLabel').textContent=A.ui?.stageLabel||'Giai đoạn'; $('#stageSelect').innerHTML=stages().map(s=>`<option value="${esc(s.id)}">${esc(s.title)}</option>`).join(''); $('#stageSelect').value=state.stage; $('#nav').innerHTML=NAV.map(n=>`<button data-view="${esc(n[0])}" class="${state.view===n[0]?'active':''}"><b>${n[1]}</b><span>${esc(n[2])}</span></button>`).join(''); const tb=$('#themeBtn'); if(tb){tb.textContent='☀️ Giao diện';tb.classList.add('theme-light-button');} }
...
function setPresentationOpen(on){
 ...
 if(flag){
  document.querySelectorAll('.learn-structure-menu[open], .storage-group-menu-wrap details[open], details.learn-structure-menu[open]').forEach(d=>d.removeAttribute('open'));
 }
}
...
function openModal(html,type='generic'){
 ...
 if(card){card.classList.toggle('presentation-card',type==='presentation');card.classList.toggle('route-card-modal',type==='route');card.classList.toggle('exam-result-card-modal',type==='exam-result');card.classList.toggle('confirm-card-modal',type==='confirm');card.scrollTop=0;}
 ...
 if(type==='presentation')setTimeout(()=>{const ps=presentationScroller(); if(ps){ps.scrollTop=0; ps.focus({preventScroll:true});}},40);
}
function closeModal(){ state.modalType=''; setPresentationOpen(false); $('#modal').classList.add('hidden'); const body=$('#modalBody'); body.innerHTML=''; body.classList.remove('presentation-body'); const card=$('#modal .modal-card'); if(card){card.classList.remove('presentation-card','route-card-modal','exam-result-card-modal','confirm-card-modal');} }
...
function overviewStats(){return [['Bài học',call('getLessons',[],DB).length],['Cú pháp',call('getVocabulary',[],DB).length],['Vấn đáp',call('getDialogues',[],DB).length],['Câu test',call('getTests',[],DB).length]]}
...
function healthSummary(){const required=['curriculum','lessons','vocab','speaking','tests','videos','handwriting','writing']; const missing=required.filter(f=>!DB[f]||dataCount(f)===0); const warnings=[]; if(!call('getLessons',[],DB).length)warnings.push('chưa có bài học'); if(!call('getVocabulary',[],DB).length)warnings.push('chưa có workflow-cú-pháp'); if(!call('getDialogues',[],DB).length)warnings.push('chưa có vấn đáp'); return {ok:!missing.length&&!warnings.length,missing,warnings};}
...
function stageReality(){
 const s=state.stage;
 if(s==='vn')return 'Bối cảnh thực tế: giai đoạn Việt Nam ưu tiên Python, SQL, Git và notebook; workflow-cú-pháp học qua bài nhỏ, không nhồi framework.';
 if(s==='prep')return 'Bối cảnh thực tế: giai đoạn dự bị củng cố ngôn ngữ toán, ký hiệu Nga, giải tích, đại số, xác suất nền để vào môn chuyên ngành không hụt nhịp.';
...
function routeBlockRoute(b,i){
 ...
 if(/workflow-cú-pháp|ngu phap|grammar|câu mẫu|cau mau|mẫu câu|mau cau|lý thuyết|ly thuyet/.test(hay))return grammarRouteForStage(currentStageId());
 ...
 if(/ứng dụng|case|ai|robot|tín hiệu|đời sống/.test(hay))return {view:'dialogue'}; if(/vấn đáp|giáo sư|hỏi|trả lời|lý thuyết/.test(hay))return {view:'learning',learnTab:'practice'};
 if(/mô phỏng|tham số|biến đầu vào|đầu ra|simulation/.test(hay))return {view:'writing'};
 if(/workflow-cú-pháp|cụm|từ\/cụm/.test(hay))return {view:'vocab'};
...
function routeBlockDetail(b,i){
 ...
 const presets=[
  ['Mở workflow bằng demo','Xem một minh họa ngắn để nắm hình học/trực giác trước khi lao vào workflow-cú-pháp.'],
  ['Đọc yêu cầu chủ động','Đọc đề 2 lượt, gạch biến, đơn vị, giả thiết và đầu ra cần chứng minh/tính.'],
  ['Code mẫu từng bước','Làm lại ví dụ mẫu, ghi rõ vì sao được phép biến đổi từng dòng.'],
  ['Vấn đáp tình huống','Tự đóng vai giảng viên hỏi: input/output, thiết kế, test, lỗi logic và ứng dụng.'],
  ['Thuật ngữ/workflow-cú-pháp phụ trợ','Chọn ít workflow-cú-pháp dùng ngay trong bài, không học bảng dài như kho lạnh ký hiệu.'],
  ['Thuật ngữ qua ví dụ mô phỏng','Chỉ rút ra 1-2 ký hiệu/quy tắc phục vụ bài vừa giải.'],
  ['Gắn với AI/robotics','Gắn kết quả với dữ liệu cảm biến, tín hiệu, điều khiển, UGV/USV hoặc báo cáo nghiên cứu.']
 ];
...
function overviewKpiCards(){
 ...
 const kpis=[
  {label:'Ứng dụng',value:dialogueTotal,unit:'case',tone:'blue',route:{view:'dialogue'}},
  {label:'Video/Audio',value:mediaTotal,unit:'nguồn',tone:'rose',route:{view:'media'}},
  {label:'Thuật ngữ/workflow-cú-pháp phụ trợ',value:stats.find(x=>x[0]==='Cú pháp')?.[1]||0,unit:'thẻ',tone:'green',route:{view:'vocab'}},
...
function overviewLearningFlow(){
 const items=[
  ['01','Mở tai','Video/demo ngắn để bắt nhịp lập trình thật.',{view:'media'}],
  ['02','Giải câu','Đọc đề, giải chậm, rồi tự trình bày lại từng bước.',{view:'dialogue'}],
  ['03','Đóng vai','Đổi góc nhìn theo bối cảnh lớp học, ký túc, giáo vụ.',{view:'dialogue'}],
  ['04','Ghi ký hiệu','Ghi lại workflow-cú-pháp và ký hiệu dễ nhầm vào sổ lỗi.',{view:'writing',mode:'handwriting'}],
  ['05','Chốt lỗi','Ghi 1 lỗi biến đổi/workflow-cú-pháp và 1 bài sẽ giải lại ngày mai.',{view:'learning',learnTab:'review'}]
 ];
 ...
 const quick=[
  ...
  ['Ghép video với bài tập','Xem xong chuyển ngay sang bài tập và tự vấn đáp.',{view:'media'},'Xem minh họa'],
...
function overviewSkillTiles(){
 const tiles=[
  ...
  {icon:'🎙️',title:'Vấn đáp bằng bước ngắn',text:'Ưu tiên trình bày được định nghĩa, giả thiết, workflow-cú-pháp và kết luận bằng 4-6 câu rõ ràng.',route:{view:'dialogue'}},
  {icon:'🧩',title:'Cú pháp dùng ngay',text:'Chỉ lấy workflow-cú-pháp phục vụ bài hôm nay, không ôm bảng workflow-cú-pháp như vác bê tông khô.',route:{view:'vocab'}},
  {icon:'✍️',title:'Mô phỏng để khóa suy luận',text:'Chạy mô phỏng nhỏ, ghi biến đầu vào, kết quả đầu ra và kết luận ứng dụng.',route:{view:'writing'}},
 ];
...
function renderOverviewReminderPanel(){
 ...
 const focusText=weak?`Vá ${weak} điểm yếu trước khi tăng tốc.`:'Nền hôm nay sạch, có thể đi tiếp theo nhịp lý thuyết - bài tập.';
 const priorities=[
  ['01','Mở tai','Video/demo ngắn 5-12 phút để lấy nhịp lập trình thật.',{view:'media'},'Xem minh họa'],
  ['02','Giải câu','Đọc case ứng dụng, xác định workflow-cú-pháp, biến đầu vào và kết quả đầu ra.',{view:'dialogue'},'Vấn đáp'],
  ['03','Dùng ngay','Đóng vai trong lớp học/ký túc/phòng giáo vụ, ưu tiên câu ngắn.',{view:'dialogue'},'Đóng vai'],
  ['04','Khóa lại','Ghi kết luận mô phỏng và ghi 1 lỗi cần sửa ngày mai.',{view:'writing',mode:'handwriting'},'Mô phỏng/kết luận']
 ];
 const status=[
  ['Ứng dụng',call('getDialogues',[],DB).length,'case',{view:'dialogue'}],
  ['Video/Audio',call('getMedia',[],DB).length,'nguồn',{view:'media'}],
  ['Thuật ngữ/workflow-cú-pháp phụ trợ',overviewStats().find(x=>x[0]==='Cú pháp')?.[1]||0,'thẻ',{view:'vocab'}],
...
function renderOverview(){
 ...
 const quick=[
  ...
  ['Giải thích ứng dụng','Chuyển workflow-cú-pháp thành bối cảnh AI/robotics và nêu giới hạn áp dụng.',{view:'dialogue'},'Vấn đáp']
 ];
 return `<div class="overview-v128 overview-v1261-closure">
  <section class="overview-mission-clean panel overview-top-only-hero v1261-hero">
    <div class="mission-copy-clean">
      ...
      <p>${esc(stageTitle())} · ưu tiên nghe, nói và video; workflow-cú-pháp/workflow-cú-pháp chỉ phụ trợ cho câu vừa dùng.</p>
    </div>
...
function routeStageSupport(stageId=currentStageId()){
 const map={
  vn:{level:'A0',track:'Cách đọc',grammarLabel:'A0/A1 · phát âm, câu tối thiểu, giống danh từ',mindmapId:'phonetics-map',mindLabel:'Phonetics · chữ, âm, trọng âm',check:'Nhìn sơ đồ âm → đọc 3 cặp âm → nhại 2 câu đã học.'},
  ...
  hk2:{level:'B2',track:'Học thuật',grammarLabel:'B2 · phong cách báo cáo, dữ liệu và thuật toán',mindmapId:'vocab-network-map',mindLabel:'Cú pháp học thuật theo mạng ngữ cảnh',check:'Chọn 1 cụm học thuật → nối sang động từ/giới từ → dùng trong câu lab.'},
...
function routeStageSupportBlock(type='grammar',stageId=currentStageId()){
 ...
 return {key:'stage_grammar_lane',_autoRouteSupport:true,minutes:10,label:'Cú pháp theo giai đoạn',action:'Mở Cú pháp đúng cấp',output:`${p.grammarLabel}. Học 1 quy tắc nhỏ qua câu vừa nghe/nói, sau đó tạo 1 câu của chính mình.`,route:grammarRouteForStage(stageId)};
}
function hasRouteSupportKind(blocks,kind){
 const re=kind==='mindmap'?/mind\s*map|mindmap|sơ đồ|check nhớ|neo nhớ|ghi nhớ/:/workflow-cú-pháp|grammar|câu mẫu|mẫu câu/;
...
function baseRouteSession(){return {order:1,date:new Date().toISOString().slice(0,10),minutes:90,sessionKind:'orientation',phase:'Làm quen A0 an toàn',output:'Mục tiêu hôm nay: đọc yêu cầu, viết code nhỏ, chạy test/log và commit artifact; chưa kiểm tra chính thức.',blocks:[{key:'video_warmup',minutes:10,label:'Làm quen mục tiêu và cách học',action:'Mở workflow bằng demo',output:'Xem một minh họa ngắn để nắm hình học/trực giác trước khi lao vào workflow-cú-pháp.'},{key:'active_listening',minutes:25,label:'Chữ cái, âm khó, cách nghe an toàn',action:'Đọc yêu cầu chủ động',output:'Đọc đề 2 lượt, gạch biến, đơn vị, giả thiết và đầu ra cần chứng minh/tính.'},{key:'shadowing',minutes:20,label:'Chào hỏi và câu lớp học đầu tiên',action:'Code mẫu từng bước',output:'Làm lại ví dụ mẫu, ghi rõ vì sao được phép biến đổi từng dòng.'},{key:'roleplay',minutes:20,label:'Nghe - nhại từng câu ngắn',action:'Vấn đáp tình huống',output:'Tự đóng vai giảng viên hỏi: input/output, thiết kế, test, lỗi logic và ứng dụng.'},{key:'write_or_type',minutes:10,label:'Ghi kết luận mô phỏng',action:'Thuật ngữ/workflow-cú-pháp phụ trợ',output:'Chọn ít workflow-cú-pháp dùng ngay trong bài, không học bảng dài như kho lạnh ký hiệu.'},{key:'reflection',minutes:5,label:'Tự phản hồi, không tính điểm',action:'Thuật ngữ qua ví dụ mô phỏng',output:'Chỉ rút ra 1-2 ký hiệu/quy tắc phục vụ bài vừa giải.'}],cards:[{title:'Video/demo mở workflow',purpose:'Xem demo/code nhỏ trước, làm quen workflow và lỗi thường gặp. Đây là cửa vào gọn cho giai đoạn Việt Nam.',limit:'1 video/audio ngắn 5-12 phút',button:'Xem/chạy ngay',route:{view:'media'}},{title:'Vấn đáp chủ lực',purpose:'Tập trả lời câu hỏi giáo sư: bản chất, workflow-cú-pháp, điều kiện áp dụng, lỗi sai và ứng dụng.',limit:'1-2 vấn đáp thực tế',button:'Vấn đáp',route:{view:'dialogue'}},{title:'Ứng dụng trong bài',purpose:'Đi từng câu hỏi theo bài học, trả lời chậm, tự đánh dấu câu trả lời đã ổn.',limit:'20-30 phút',button:'Mở ứng dụng',route:{view:'learning',learnTab:'practice'}},{title:'Mô phỏng và ghi kết luận',purpose:'Chạy một mô phỏng nhỏ, ghi biến đầu vào, kết quả đầu ra và kết luận ứng dụng.',limit:'5-10 phút',button:'Mở mô phỏng',route:{view:'writing'}},{title:'Cú pháp & workflow-cú-pháp phụ trợ',purpose:'Chỉ lấy 5-8 lệnh/thuật ngữ phục vụ bài code hôm nay. Không biến cú pháp thành kho lạnh.',limit:'5-8 cụm + 1 mẫu',button:'Học phụ trợ',route:{view:'vocab'}},{title:'Ôn tập nhẹ cuối buổi',purpose:'Tự hỏi: nghe lại được chưa, nhại được chưa, viết/gõ lại được chưa. Chưa cần kiểm tra chính thức.',limit:'3 câu tự phản hồi',button:'Mở ôn tập',route:{view:'learning',learnTab:'review'}}],antiCrammingNote:'Giai đoạn đầu không nhồi test; mỗi buổi phải có code thật, test/log, ghi README và tự phản hồi.',pedagogyNote:'Học theo vòng: đọc yêu cầu → code nhỏ → test/log → commit → tự sửa lỗi.'} }
function routeBlockDefaults(kind='study'){
 const map={
  orientation:[['video_warmup','Làm quen mục tiêu và cách học',10,'Mở workflow bằng demo','Xem một minh họa ngắn để nắm hình học/trực giác trước khi lao vào workflow-cú-pháp.'],['active_listening','Chữ cái, âm khó, cách nghe an toàn',25,'Đọc yêu cầu chủ động','Đọc đề 2 lượt, gạch biến, đơn vị, giả thiết và đầu ra cần chứng minh/tính.'],['shadowing','Chào hỏi và câu lớp học đầu tiên',20,'Code mẫu từng bước','Làm lại ví dụ mẫu, ghi rõ vì sao được phép biến đổi từng dòng.'],['roleplay','Nghe - nhại từng câu ngắn',20,'Vấn đáp tình huống','Tự đóng vai giảng viên hỏi: input/output, thiết kế, test, lỗi logic và ứng dụng.'],['write_or_type','Ghi kết luận mô phỏng',10,'Thuật ngữ/workflow-cú-pháp phụ trợ','Chọn ít workflow-cú-pháp dùng ngay trong bài, không học bảng dài như kho lạnh ký hiệu.'],['reflection','Tự phản hồi, không tính điểm',5,'Thuật ngữ qua ví dụ mô phỏng','Chỉ rút ra 1-2 ký hiệu/quy tắc phục vụ bài vừa giải.']],
  foundation:[['video_context','Video/audio theo ngữ cảnh',15,'Nghe mở tai','Nhận diện nhịp nói và cụm lặp lại trong tình huống thật.'],['active_listening','Nghe hiểu ý chính',20,'Đọc yêu cầu chủ động','Bắt ý chính, khoanh âm/từ chưa rõ, chưa sa đà tra cứu.'],['repeat_shadow','Nhại và chỉnh trọng âm',20,'Shadowing','Nói lại từng câu, giữ trọng âm và nhịp câu.'],['guided_dialogue','Đổi góc nhìn vấn đáp có hướng dẫn',20,'Đóng vai','Dùng câu mẫu trong lớp học, ký túc hoặc phòng giáo vụ.'],['vocab_minimum','Cú pháp/ký hiệu tối thiểu',10,'Học phụ trợ','Chọn cụm cần dùng ngay trong vấn đáp.'],['write_spoken_sentence','Ghi kết luận mô phỏng',5,'Gắn workflow-cú-pháp-mô phỏng-kết luận','Ghi lại kết luận mô phỏng để khóa suy luận và phát hiện lỗi.']],
  study:[['listen_again','Đọc lại và bắt cấu trúc bài',15,'Đọc yêu cầu chủ động','Xác định giả thiết, biến, đơn vị và đầu ra cần chứng minh/tính.'],['video_context','Video/audio theo chủ đề hôm nay',20,'Mở ngữ cảnh','Xem/nghe nguồn ngắn để đặt câu vào đời sống thật.'],['dialogue','Ứng dụng và phản biện workflow-cú-pháp',30,'Đổi góc nhìn','Nói rõ workflow-cú-pháp dùng vào đâu, điều kiện nào làm kết quả sai.'],['shadowing','Kiểm chứng bằng ví dụ số',15,'Tính thử / mô phỏng nhỏ','Nói chồng theo audio, giữ nhịp trước khi phân tích.'],['vocab_chunks','Thuật ngữ/workflow-cú-pháp dùng ngay',10,'Chọn workflow-cú-pháp sống','Chọn ít workflow-cú-pháp nhưng phải nêu được ý nghĩa, điều kiện và ứng dụng.'],['write_or_type','Ghi 3-5 nhận xét mô phỏng',10,'Chốt bằng kết luận','Ghi kết luận để khóa suy luận và phát hiện lỗi mô hình.']],
  review_consolidation:[['listen_wrong_lines','Nghe lại câu từng sai/chưa rõ',20,'Sửa lỗi nghe','Tập trung vào câu sai, không mở quá nhiều nội dung mới.'],['roleplay','Đóng vai trọng điểm',25,'Nói lại trong ngữ cảnh','Đổi góc nhìn A/B với những mẫu câu còn yếu.'],['video_rewatch','Xem/nghe lại video liên quan',15,'Ôn bằng ngữ cảnh','Nghe lại nguồn cũ để củng cố phản xạ.'],['pronunciation_repair','Sửa phát âm và cụm dùng sai',20,'Chữa âm','Tách lỗi phát âm, nói lại chậm rồi tăng tốc.'],['mini_check','Mini-check củng cố',10,'Tự kiểm nhẹ','Tự hỏi nhanh, không thay kiểm tra chính thức.']],
...
function routeCardsFallback(kind='study'){
 ...
 if(kind==='review_consolidation')return [{title:'Ôn gián đoạn',purpose:'Nhắc lại đúng khoảng cách để nhớ lâu, ưu tiên câu đã nghe/nói.',limit:'15-25 thẻ/câu',button:'Ôn từ/câu',route:{view:'vocab'}},{title:'Đổi góc nhìn trọng điểm',purpose:'Nói lại tình huống còn yếu, sửa câu chưa tròn nghĩa.',limit:'1-2 vấn đáp',button:'Vấn đáp',route:{view:'dialogue'}},{title:'Mini-check không tính điểm',purpose:'Tự kiểm nhẹ để biết đã sẵn sàng chưa, chưa thay kiểm tra chính thức.',limit:'5-10 câu',button:'Mở ôn tập',route:{view:'learning',learnTab:'review'}}];
 return base;
}
...
function renderRouteToday(s){
 ...
 const merged=details.map((b,i)=>{
   ...
   const routeToday=route?.learnTab==='exam'?routeExamPayloadForToday({...s,...b,...c,...route},s,i+1):{...route,routeSource:'today_schedule',stage:currentStageId(),part:gatePart(),sessionKey:sessionKey(s),scheduleStep:i+1};
...
function renderRoutePracticalStrip(s){
 s=normalizeRouteSession(s);
 const p=routeStageSupport(currentStageId());
 return `<div class="today-practical-strip route-modal-practical"><article><b>Đầu ra cần có</b><span>${esc(s.output||'Làm quen, nghe-nhại, viết/gõ; chưa kiểm tra chính thức.')}</span></article><article><b>Cú pháp theo giai đoạn</b><span>${esc(p.grammarLabel)} · chỉ lấy 1 quy tắc nhỏ để dùng ngay trong câu.</span></article><article><b>Check nhớ Mind map</b><span>${esc(p.check)} Đây là nhắc nhớ cuối buổi, không học lan man.</span></article></div>`
}
function renderRouteOutputChecklist(s){
 const p=routeStageSupport(currentStageId());
 const items=[
  ['Nghe','Đã xem/chạy ít nhất 1 demo lập trình và hiểu luồng input-output.',{view:'media'}],
  ['Nói','Đã nhại/đóng vai tối thiểu 3-5 câu ngắn.',{view:'dialogue'}],
  ['Cú pháp','Đã rút 1 mẫu workflow-cú-pháp đúng cấp và tự tạo 1 câu.',grammarRouteForStage(currentStageId())],
  ['Mind map check',p.check,mindmapRouteForStage(currentStageId())],
  ['Mô phỏng/kết luận','Đã chạy mô phỏng và ghi 1 kết luận định lượng.',{view:'writing',mode:'handwriting'}],
...
function renderRouteCompletionPlan(s){
 ...
 const rows=[
  ...
  ['Nếu hụt giờ','Giữ video + nhại câu; giảm workflow-cú-pháp/workflow-cú-pháp xuống mức phụ trợ.','route-extra-time'],
...
function renderRouteClosurePanel(s){
 s=normalizeRouteSession(s);
 const items=[
  ['Nghe','Đã xem/chạy ít nhất 1 demo lập trình, hiểu input-output và lỗi chính.',{view:'media'}],
  ['Nói','Đã nhại/đóng vai tối thiểu 3-5 câu ngắn.',{view:'dialogue'}],
  ['Mô phỏng/kết luận','Đã ghi lại biến đầu vào, kết quả đầu ra và lỗi cần tránh.',{view:'writing',mode:'handwriting'}],
...
function learningListForMode(){
 ...
 if(mode==='practice'){
   ...
   const item=d=>{const group=A.dialogueGroup?.(d)||d.group||d.applicationDomain||'ứng dụng'; const diff=A.dialogueDifficulty?.(d)||d.difficulty||d.level||'applied'; return {group,diff,title:A.dialogueTitle?.(d)||d.title||'Ứng dụng',sub:A.dialogueSubtitle?.(d)||d.purpose||d.applicationDomain||''};};
   return {title:'Ứng dụng '+ctx.id, hint:'Chọn case ứng dụng để nối workflow-cú-pháp với AI, tín hiệu, robot, đời sống và yêu cầu tốt nghiệp.', items:ds.map((d,i)=>{const it=item(d); return `<button class="learn-item math-app-list-item clean-left-item ${active===d?'active':''}" data-practice-dialogue="${esc(d.id||d.title)}"><span>${String(i+1).padStart(2,'0')}</span><b>${esc(it.title)}</b><em>${esc(clip(it.sub,74))}</em></button>`}).join('')||'<div class="note">Bài này chưa có case ứng dụng đúng lessonId hoặc bộ lọc đang quá hẹp.</div>'};
 }
 if(mode==='review')return {title:'Ôn tập', hint:'Các bộ lọc và câu hỏi nằm trong khung chính.', items:'<div class="note">Ôn tập hiển thị ở khung bên phải. Hàng nút phía trên giữ cùng bố cục với Lý thuyết, Bài tập và Ứng dụng.</div>'};
...
function renderLearning(){
 ...
 const modeMeta={
  ...
  practice:{label:'Ứng dụng',kicker:'Cú pháp → AI/Bauman',hint:'Mỗi case phải nêu được: đời sống/AI, workflow-cú-pháp dùng, biến đầu vào, kết quả, giới hạn và tiêu chí tốt nghiệp.'}
 }[state.learnTab]||{label:list.title||'Học tập',kicker:'Học theo nhịp',hint:'Chọn đúng chế độ ở thanh trên.'};
 const ctxDialogues=getPracticeDialogues();
...
function renderTheory(lessons,concepts,currentLessonArg){
 /* INFO_RENDERER_CLEAN: metadata/tags/keywords/related concepts kept out of lesson and presentation body, matching fixed Math INFO file. */
 const lesson=currentLessonArg||currentLesson();
 const {slides,idx}=clampSlideIndex(lesson);
 const slide=slides[idx]||{};
 const theorySimBtn=(A.subjectKind==='mathematics'||A.subjectKind==='programming')?`<button class="btn gold math-sim-open-btn theory-open" data-open-simulation="theory" data-sim-target="${esc(lessonKey(lesson))}"><span>↗</span><b>Mở: Mô phỏng</b></button>`:'';
 return `<section class="panel lesson-reader v1257-theory-main v1261-theory-main v1271-theory-main v1295-theory-tight">
   <div class="lesson-reader-top v1257-reader-top v1271-theory-head v1283-theory-head v1295-theory-head">
     ...
     <div class="lesson-tools v1271-theory-tools"><button class="btn" data-act="prev-slide">←</button><span class="chip">${idx+1}/${slides.length||1}</span><button class="btn" data-act="next-slide">→</button>${theorySimBtn}<button class="btn primary" data-act="open-present">⛶ Trình chiếu</button></div>
   </div>
...
function mathTermsTable(terms){
 ...
 if(!arrTerms.length)return '<div class="note">Chưa có bảng thuật ngữ Việt - Nga - Anh cho mục này.</div>';
...
function lessonByIdForMath(id){return getLessons().find(l=>lessonKey(l)===id)||{};}
function mathStageBadge(item){return `${esc(stageTitle(item?.stage||state.stage))}`;}
function renderApplicationStudy(){
 const ctx=activeLessonContext();
 const list=getPracticeDialogues();
 if(!Array.isArray(DB['dialogue-bauman-az'])||!DB['dialogue-bauman-az'].length)return `<section class="panel math-app-empty"><span class="chip">ỨNG DỤNG</span><h3>Chưa có dữ liệu ứng dụng</h3><p>Hãy kiểm tra data/dialogue-bauman-az.json trong tab Dữ liệu.</p></section>`;
 ...
 const steps=arr(active.applicationSteps||active.aiPipeline||active.turns).slice(0,6);
 const questions=arr(active.analysisQuestions||active.professorQuestions||active.reflectionQuestions||[]).slice(0,6);
 const formula=active.formulaUsed||active.formula||active.coreFormula||lesson.formula||'';
 const domain=active.applicationDomain||active.purpose||'Ứng dụng phần mềm vào AI/Bauman';
 const need=active.aiGraduationNeed||active.graduationMastery||'Muốn bảo vệ tốt cần hiểu workflow-cú-pháp ở mức: giải thích được mục tiêu, biến đầu vào, giới hạn, lỗi và cách kiểm chứng bằng mô phỏng.';
 const real=active.realLifeScenario||active.realLife||'Đời sống/kỹ thuật: biến hiện tượng thật thành dữ liệu đo được, sau đó dùng workflow-cú-pháp để ra quyết định.';
 const inputs=arr(active.inputs||active.variables||[]).slice(0,4);
 const outputs=arr(active.outputs||active.expectedOutputs||[]).slice(0,4);
 const turnRows=arr(active.turns).slice(0,5).map((t,i)=>`<article><b>${esc(t.speaker||('Bước '+(i+1)))}</b><p>${esc(t.vi||t.ru||t.text||'')}</p></article>`).join('');
 return `<section class="math-app-workbench">
   <header class="math-app-hero panel"><div><span class="chip">🧩 ỨNG DỤNG · ${mathStageBadge(active)}</span><h3>${esc(active.title||'Ứng dụng Programming')}</h3><p>${esc(domain)}</p></div><button class="btn primary math-sim-open-btn app-open" data-open-simulation="application" data-sim-target="${esc(active.id||active.targetId||active.lessonId||ctx.id)}"><span>↗</span><b>Mở: Mô phỏng</b></button></header>
   <div class="math-app-grid">
     <article class="panel math-app-card main-case"><span>Cú pháp dùng trong AI/Bauman</span><h4>${esc(formula||'Cú pháp đang cập nhật')}</h4><p>${esc(need)}</p></article>
     <article class="panel math-app-card"><span>Đối ứng đời sống/kỹ thuật</span><p>${esc(real)}</p></article>
     <article class="panel math-app-card"><span>Giai đoạn lộ trình</span><p>${esc(stageTitle(active.stage||lesson.stage||state.stage))}</p><small>${esc(lesson.title||ctx.id||'Bài học hiện tại')}</small></article>
   </div>
   <section class="panel math-ai-pipeline"><header><span class="chip">AI PIPELINE</span><h4>Từ workflow-cú-pháp đến mô hình</h4></header><div class="math-pipeline-steps">${(steps.length?steps:arr(active.turns).slice(0,4)).map((x,i)=>{const text=typeof x==='string'?x:(x.vi||x.ru||x.text||x.step||''); return `<article><i>${String(i+1).padStart(2,'0')}</i><p>${esc(text)}</p></article>`}).join('')||'<article><p>Chưa có pipeline, hãy bổ sung trong JSON ứng dụng.</p></article>'}</div></section>
   <section class="math-app-two-col"><article class="panel"><h4>Biến đầu vào → kết quả</h4><div class="math-io-grid"><div><b>Input</b>${inputs.length?inputs.map(x=>`<span>${esc(x.label||x.name||x)}</span>`).join(''):'<span>Biến, tham số, dữ liệu đo</span>'}</div><div><b>Output</b>${outputs.length?outputs.map(x=>`<span>${esc(x.label||x.name||x)}</span>`).join(''):'<span>Dự đoán, sai số, tiêu chí tối ưu</span>'}</div></div></article><article class="panel"><h4>Thuật ngữ Việt - Nga - Anh</h4>${mathTermsTable(terms)}</article></section>
   <section class="panel math-mastery-panel"><header><span class="chip">BẰNG GIỎI</span><h4>Cần hiểu sâu những gì?</h4></header><div class="math-mastery-grid"><article><b>1. Bản chất</b><p>Giải thích bằng lời trước khi viết workflow-cú-pháp.</p></article><article><b>2. Điều kiện</b><p>Nêu rõ giả thiết, miền giá trị, kích thước và đơn vị.</p></article><article><b>3. Sai số</b><p>Biết khi nào dữ liệu nhiễu làm kết luận lệch.</p></article><article><b>4. Ứng dụng</b><p>Nối được với AI, tín hiệu, robotics, НИР/ВКР.</p></article></div></section>
   ${questions.length?`<section class="panel math-app-questions"><h4>Câu hỏi tự kiểm ứng dụng</h4><ol>${questions.map(q=>`<li>${esc(q)}</li>`).join('')}</ol></section>`:''}
 </section>`;
}
...
function speechMapLineButton(active,t,i){
 ...
 const label=mine?'Lượt của bạn':(role==='all'?'Nhại':'Gợi ý câu hỏi');
...
function renderSpeakingMapModal(){
 ...
 const title=A.dialogueTitle?.(active)||active?.title||'Bản đồ vấn đáp';
 return `<div class="modal-body v1289-speaking-map-modal"><header><span class="chip">🧭 BẢN ĐỒ CÂU NÓI</span><h3>${esc(title)}</h3><p>Chọn một câu để luyện ngay. Bản đồ được đưa vào popup để không lấn phần nói chính.</p></header><div class="dialogue-lines v1289-map-grid">${turns.length?turns.map((t,i)=>speechMapLineButton(active,t,i)).join(''):'<div class="note">Không có câu nói trong vấn đáp đúng bài này.</div>'}</div><div class="modal-actions"><button class="btn primary" data-act="modal-close">Đóng</button></div></div>`
}
function normalizeSlideValue(v){
 if(v==null||v==='')return '';
 if(Array.isArray(v))return v.map(normalizeSlideValue).filter(Boolean).join('\n');
 if(typeof v==='object')return Object.entries(v).filter(([k])=>!['id','stage','lessonId','slides','blocks','type','kind','tags','keywords','az_tags','integration_tags','vocabulary_seed_ru','_internalMetaForSearchOnly','searchOnly','metadata'].includes(k)).map(([k,val])=>`${k}: ${normalizeSlideValue(val)}`).filter(x=>!x.endsWith(': ')).join('\n');
 return str(v);
}
...
function startLineRecording(){
 ...
 state.speechRecording=true; save(); render(); toast('Đang nghe phần trình bày Programming của bạn...');
 rec.onresult=ev=>{const transcript=ev.results?.[0]?.[0]?.transcript||''; const score=Math.round(speechSimilarity(transcript,target)*100); const store=activeSpeechResults(); store[dialogueLineKey(d,idx)]={score,transcript,target,at:Date.now(),ok:score>=70}; state.speechRecording=false; save(); render(); toast(score>=70?'Đạt bước này, có thể qua bước tiếp':'Chưa đạt, trình bày lại bước này'); if(score>=70&&state.speechAutoNext)setTimeout(()=>moveDialogueLine(1),520)};
...
function renderRelatedConcepts(concepts,limit=3){return ''}
...
function vocabDialogueExampleHtml(info){
 ...
 return `<section class="v1313-vocab-dialogue-example" aria-label="Ví dụ vấn đáp thực tế"><div><span class="chip">Ví dụ</span><h4>Hội thoại thực tế có dùng từ/cụm này</h4></div><div class="dialogue-example-lines">${lines.map(line=>{const parts=str(line).split(':'); const role=parts.length>1?parts.shift():'A'; const text=parts.join(':').trim()||line; return `<p><b>${esc(role.trim())}</b><span>${esc(text)}</span></p>`}).join('')}</div></section>`;
}

function vocabVisualHtml(info,back=false){
 const tags=arr(info.tags).filter(t=>!['vn','prep','hk1','hk2','hk3','hk4','programming','bauman','all','custom'].includes(lower(t))).slice(0,4);
...
function renderDeepDialoguePanel(active){
 ...
 if(!optionalReady('deep-speaking-bauman'))return `<details class="deep-dialogue-panel deep-nine-panel"><summary><span>🎙️ Phản biện sâu case này</span><b>Chưa tải Deep Speaking</b></summary><div class="deep-nine-body"><p>Rapid drill, shadowing, monologue và Q&A chỉ tải khi bạn cần.</p><button class="btn primary" data-load-optional="deep-speaking-bauman">Tải Deep Speaking</button></div></details>`;
 if(!optionalReady('speaking-link-index'))return `<details class="deep-dialogue-panel deep-nine-panel"><summary><span>🔗 Cầu nối Deep</span><b>Chưa tải link-index</b></summary><div class="deep-nine-body"><p>Tải link-index để nối vấn đáp với bài luyện sâu tương ứng.</p><button class="btn primary" data-load-optional="speaking-link-index">Tải cầu nối</button></div></details>`;
 const units=deepUnitsForDialogue(active);
 if(!units.length)return `<details class="deep-dialogue-panel deep-nine-panel"><summary><span>🎙️ Luyện sâu</span><b>Chưa có phản biện trực tiếp</b></summary><div class="deep-nine-body"><p>Case vẫn xem bình thường. Phản biện sâu sẽ hiện khi có unit cùng bài/tag.</p></div></details>`;
...
function renderDialogue(){
 const all=byStage(getBasicSpeakingDialogues());
 const groups=uniq(all.map(x=>A.dialogueGroup?.(x)||x.group||'vấn đáp giáo sư'));
 const diffs=uniq(all.map(x=>A.dialogueDifficulty?.(x)||x.difficulty||x.level||'academic'));
 ...
 let list=all;
 if(state.dialogueGroup!=='all')list=list.filter(x=>(A.dialogueGroup?.(x)||x.group||'vấn đáp giáo sư')===state.dialogueGroup);
 if(state.dialogueDifficulty!=='all')list=list.filter(x=>(A.dialogueDifficulty?.(x)||x.difficulty||x.level||'academic')===state.dialogueDifficulty);
 if(state.dialogueQuery)list=list.filter(x=>lower(textOf(x)).includes(lower(state.dialogueQuery)));
 ...
 const title=A.dialogueTitle?.(active)||active?.title||'Vấn đáp giáo sư';
 const purpose=A.dialogueSubtitle?.(active)||active?.purpose||'Câu hỏi lý thuyết thực tế giáo sư có thể hỏi trong lớp, seminar hoặc bảo vệ.';
 const questions=arr(active.professorQuestions).slice(0,8);
 const answerPlan=arr(active.expectedAnswerStructure).slice(0,8);
 const currentVi=hideVi?'':dialogueVi(line);
 const cueText=role==='all'?'Đọc câu hỏi và tự trả lời theo cấu trúc':(isMine?'Đến lượt bạn trả lời':'Nghe/đọc vai còn lại để giữ mạch vấn đáp');
 const roleName=role==='all'?'Giáo sư + học viên':`Luyện vai ${role}`;
 const roleLine=(t,i)=>{const r=dialogueRoleOf(t,i), mine=role!=='all'&&role===r, res=speakingResultFor(active,i); return `<button class="dialogue-line oral-map-line ${i===idx?'active':''} ${mine?'my-role':''} ${res?.ok?'spoken-ok':res?'spoken-try':''}" data-line="${i}"><span class="speaker">${esc(r)}</span><b>${esc(dialogueText(t))}</b><small>${String(i+1).padStart(2,'0')} · ${mine?'Cần trả lời':'Câu hỏi/gợi ý'}</small></button>`};
 return `<div class="oral-defense-wrap">
   <section class="panel oral-defense-hero"><div><span class="chip">🎙️ VẤN ĐÁP · ${esc(stageTitle(active.stage||state.stage))}</span><h3>${esc(title)}</h3><p>${esc(purpose)}</p></div><div class="oral-score"><b>${prog.percent}%</b><span>${prog.ok}/${prog.total} lượt đạt</span></div></section>
   <div class="oral-defense-layout">
     <aside class="panel oral-defense-list"><div class="toolbar compact-toolbar"><div><h3>Kho câu hỏi giáo sư</h3><p>${list.length}/${all.length} bộ theo giai đoạn</p></div></div><div class="dialogue-picker-tools"><select class="input" data-input="dialogueGroup"><option value="all">Tất cả nhóm</option>${groups.map(g=>`<option value="${esc(g)}" ${state.dialogueGroup===g?'selected':''}>${esc(g)}</option>`).join('')}</select><select class="input" data-input="dialogueDifficulty"><option value="all">Tất cả mức</option>${diffs.map(g=>`<option value="${esc(g)}" ${state.dialogueDifficulty===g?'selected':''}>${esc(g)}</option>`).join('')}</select><input class="input" data-input="dialogueQuery" value="${esc(state.dialogueQuery)}" placeholder="Tìm lý thuyết, workflow-cú-pháp, thuật ngữ..."></div><div class="oral-list-scroll">${list.slice(0,140).map(d=>`<button class="item-card oral-defense-item ${active===d?'active':''}" data-dialogue="${esc(d.id||d.title)}"><b>${esc(A.dialogueTitle?.(d)||d.title||'Vấn đáp')}</b><small>${esc(stageTitle(d.stage||''))} · ${esc(A.dialogueGroup?.(d)||d.group||'')}</small></button>`).join('')||'<div class="note">Chưa có câu hỏi vấn đáp.</div>'}</div></aside>
     <main class="panel oral-defense-main">
       <header class="oral-defense-toolbar"><div><span class="chip">${esc(active.lessonId||active.id||'ORAL')}</span><h4>${esc(lineRole)} · Lượt ${turns.length?idx+1:0}/${turns.length||0}</h4></div><div><button class="btn ${role==='all'?'active':''}" data-act="role-all">Toàn bộ</button><button class="btn ${role==='GS'?'active':''}" data-act="role-a">Giáo sư</button><button class="btn ${role==='HV'?'active':''}" data-act="role-b">Học viên</button><button class="btn" data-act="toggle-vi">${hideVi?'Hiện diễn giải':'Ẩn diễn giải'}</button></div></header>
       <article class="oral-current-card ${isMine?'student-turn':'listener-turn'}"><span>${esc(cueText)}</span><h3>${esc(targetText||'Chọn câu hỏi ở danh sách bên trái.')}</h3>${currentVi?`<p>${esc(currentVi)}</p>`:''}<button class="btn green speech-ok-corner" data-act="mark-line-ok">✓ Trả lời ổn</button></article>
       <div class="v1294-speech-actions oral-actions"><button class="btn" data-act="prev-line">← Câu trước</button><button class="btn green" data-act="speak-line">🔊 Nghe câu</button><button class="btn" data-act="speak-line-slow">🐢 Chậm</button><button class="btn" data-act="record-line">🎙️ Tập trả lời</button><button class="btn primary" data-act="next-line">Câu tiếp →</button></div>
       <div class="oral-feedback"><b>${result?`Điểm trả lời: ${result.score}%`:'Khung trả lời chuẩn'}</b><span>${result?esc(speechFeedback(result.score)):'Bản chất → workflow-cú-pháp/biến → điều kiện áp dụng → độ nhạy/sai số → ứng dụng AI/Bauman → thuật ngữ Việt-Nga-Anh.'}</span></div>
       <section class="oral-structure-grid"><article><h4>Cấu trúc câu trả lời</h4><ol>${answerPlan.length?answerPlan.map(x=>`<li>${esc(x)}</li>`).join(''):'<li>Bản chất</li><li>Cú pháp</li><li>Điều kiện áp dụng</li><li>Ứng dụng</li>'}</ol></article><article><h4>Thuật ngữ Việt - Nga - Anh</h4>${mathTermsTable(active.terminologyTerms)}</article></section>
       <details class="v1294-speech-map oral-map" open><summary>🧭 Bản đồ vấn đáp <span>${turns.length?idx+1:0}/${turns.length||0}</span></summary><div class="v1294-speech-map-grid">${turns.length?turns.map(roleLine).join(''):'<div class="note">Chưa có lượt hỏi/đáp.</div>'}</div></details>
     </main>
   </div>
   ${questions.length?`<section class="panel oral-professor-bank"><h4>Câu giáo sư hay hỏi</h4><div>${questions.map(q=>`<article>${esc(q)}</article>`).join('')}</div></section>`:''}
 </div>`;
}

function handwritingText(item){return A.handwritingPrint?.(item)||item.print||item.text||item.letter||'А а'}
function handwritingModeName(item){const m=item?.mode||'alphabet'; return ({alphabet:'Chữ cái',word:'Cú pháp/ký hiệu từ',sentence:'Câu thực tế',academic:'Học thuật'})[m]||m}
...
function handwritingSteps(item){
 ...
 return [
  ...
  isWord?'Trình bày từng chữ từ trái sang phải, giữ khoảng cách giữa các chữ bằng nửa thân chữ.':'Trình bày nét chính trước, giữ độ cao và độ nghiêng giống mẫu.',
...
function handwritingStrokeSteps(item){
 ...
 return [
  ...
  {title:'Nét chính',detail:'Trình bày nét dài hoặc thân chữ trước, đi chậm để không vỡ dáng.'},
  {title:'Nét phụ',detail:'Thêm móc, vòng hoặc gạch ngang; giữ điểm nối mềm, không tô đi tô lại.'},
  {title:'Ghép chữ',detail:`Trình bày đủ mẫu ${txt}, so khoảng cách giữa chữ hoa, chữ thường và khoảng nghỉ.`},
...
function writingPracticeName(mode){return ({view:'Nhìn mẫu tay',trace:'Tô theo nét',free:'Trình bày vào vở'})[mode||'trace']||'Tô theo nét'}
...
function mathTerminologyPanel(item,line){
 if(!(A.subjectKind==='mathematics'||A.subjectKind==='programming'))return '';
 const terms=arr(item?.terminologyTerms||item?.terms||item?.terminology).slice(0,6);
 if(!terms.length)return '';
 const rows=terms.map(t=>`<article><b>${esc(t.vi||t.term||'Thuật ngữ')}</b><span>${esc(t.ru||'')}</span><small>${esc(t.en||'')}</small></article>`).join('');
...
function mathSimKind(s){const k=str(s?.simKind||s?.simulationKind||s?.group||s?.type||'').toLowerCase(); return k.includes('application')||k.includes('ứng dụng')?'application':'theory';}
function mathSimTarget(s){return str(s?.targetId||s?.sourceApplicationId||s?.sourceLessonId||s?.lessonId||s?.id||'');}
function mathSimSourceLabel(kind){return kind==='application'?'Mô phỏng ứng dụng':'Mô phỏng lý thuyết';}
function openMathSimulation(kind,target){
 const mode=kind==='application'?'application':'theory';
 state.view='writing'; state.simulationKind=mode; state.simulationTargetId=str(target||'');
 const all=arr(DB.simulations); const found=all.find(s=>mathSimKind(s)===mode&&(mathSimTarget(s)===state.simulationTargetId||str(s.lessonId)===state.simulationTargetId));
 if(found)state.writingIndex=Math.max(0,all.indexOf(found));
 save(); render();
}
function simCanvaBoard(active,kind){
 const panels=arr(active?.canvaBoard?.panels).length?arr(active.canvaBoard.panels):(kind==='application'?['Hiện tượng','Dữ liệu','Cú pháp','AI output','Quyết định']:['Định nghĩa','Cú pháp','Biến','Đồ thị','Kết luận']);
 ...
 return `<section class="mathlab-canva-board panel ${kind==='application'?'app-board':'theory-board'}">
   <header><div><span class="chip">CANVA VISUAL BOARD</span><h4>${kind==='application'?'Storyboard ứng dụng':'Blueprint lý thuyết'}</h4><p>${esc(active?.canvaBoard?.visualHint||'Bảng thị giác giúp nhìn luồng biến đổi trước khi đọc số.')}</p></div><img src="assets/sim_canva_programming_lab.svg" alt="Programming simulation visual"></header>
...
function mathSimulationCategoryMenu(kind,list,scoped,raw){
 const choices=[
  {id:'theory',icon:'∑',title:'Mô phỏng lý thuyết',sub:'Theo chủ đề, bài học, workflow-cú-pháp và biến đầu vào'},
  {id:'application',icon:'▣',title:'Mô phỏng ứng dụng',sub:'Theo case đời sống, AI, tín hiệu, robot và ВКР'}
 ];
...
function simRangeHtml(v,i){
 const min=Number(v?.min??0), max=Number(v?.max??100), def=Number(v?.default??50); const pct=max>min?Math.round((def-min)*100/(max-min)):def;
 return `<article class="mathlab-var-card" style="--p:${Math.max(0,Math.min(100,pct))}%"><label>${esc(v?.label||v?.name||('Biến '+(i+1)))}</label><div class="range-shell"><i class="range-fill" style="width:${Math.max(0,Math.min(100,pct))}%"></i><input class="mathlab-range" type="range" min="${esc(min)}" max="${esc(max)}" value="${esc(def)}" oninput="this.previousElementSibling.style.width=((this.value-this.min)*100/(this.max-this.min))+'%'; this.closest('article').querySelector('output').textContent=this.value"></div><small>min ${esc(min)} · default <output>${esc(def)}</output> · max ${esc(max)}</small><p>${esc(v?.effect||'Đổi biến và quan sát đầu ra.')}</p></article>`;
}
function renderMathSimulationWorkspace(){
 const raw=byStage(arr(DB.simulations));
 if(!raw.length)return `<section class="panel learning-recovery-card"><span class="chip">🔬 MÔ PHỎNG</span><h3>Chưa có mô phỏng</h3><p>Hãy thêm dữ liệu vào data/simulations.json.</p></section>`;
 const kind=state.simulationKind==='application'?'application':'theory';
 const q=lower(state.writingQuery||'');
 const scoped=raw.filter(s=>mathSimKind(s)===kind);
 let list=q?scoped.filter(s=>lower([s.id,s.title,s.purpose,s.applicationDomain,s.formula,s.lessonId,s.targetId,JSON.stringify(s.variables||[])].join(' ')).includes(q)):scoped;
 if(!list.length)list=scoped.length?scoped:raw;
 let active=null;
 if(state.simulationTargetId)active=list.find(s=>mathSimTarget(s)===state.simulationTargetId||str(s.lessonId)===state.simulationTargetId)||raw.find(s=>mathSimKind(s)===kind&&(mathSimTarget(s)===state.simulationTargetId||str(s.lessonId)===state.simulationTargetId));
 active=active||arr(DB.simulations)[state.writingIndex]||list[0]||{};
 if(mathSimKind(active)!==kind)active=list[0]||active;
 const all=arr(DB.simulations); state.writingIndex=Math.max(0,all.indexOf(active)); state.simulationTargetId=mathSimTarget(active);
 const vars=arr(active.variables).slice(0,8);
 const effects=arr(active.inputEffects).slice(0,8);
 const steps=arr(active.explorationSteps).slice(0,8);
 const outputs=arr(active.expectedOutputs).slice(0,6);
 const file=active.file||'';
 const src=file?(file.includes('?')?file+'&':file+'?')+'kind='+encodeURIComponent(kind)+'&id='+encodeURIComponent(active.id||''):'';
 const status=[['Mode',mathSimSourceLabel(kind)],['Stage',stageTitle(active.stage||state.stage)],['Lesson',active.lessonId||active.id],['Variables',String(vars.length)]];
 const simList=list.slice(0,160).map(s=>`<button class="mathlab-sim-item ${s===active?'active':''}" data-write-index="${all.indexOf(s)}"><span>${esc(s.lessonId||s.id)}</span><b>${esc(clip(s.title||'Mô phỏng',68))}</b><small>${esc(clip(s.applicationDomain||s.purpose||'',92))}</small></button>`).join('');
 const categoryMenu=mathSimulationCategoryMenu(kind,list,scoped,raw);
 return `<div class="mathlab-shell mathlab-v9 mathlab-v10 ${kind==='application'?'mode-application':'mode-theory'}">
   <section class="panel mathlab-command mathlab-command-v10"><div class="mathlab-command-copy"><span class="chip">🔬 SIMULATION LAB · ${esc(mathSimSourceLabel(kind))}</span><h3>Phòng lab giả lập Programming → AI/Bauman</h3><p>${kind==='application'?'Mô phỏng ứng dụng biến workflow-cú-pháp thành tình huống đời sống/kỹ thuật, AI pipeline và quyết định mô hình.':'Mô phỏng lý thuyết bám từng chủ đề/bài học: đổi biến đầu vào, quan sát đồ thị/kết quả, đọc độ nhạy bằng workflow-cú-pháp.'}</p></div><div class="mathlab-command-tools">${categoryMenu}<input class="input" data-input="writingQuery" value="${esc(state.writingQuery||'')}" placeholder=">> search simulation, formula, AI application..."></div></section>
   <div class="mathlab-layout">
     <aside class="panel mathlab-sidebar"><header><b>Workspace</b><span>${list.length}/${scoped.length||raw.length}</span></header><div class="mathlab-list">${simList}</div></aside>
     <main class="panel mathlab-main">
       <header class="mathlab-titlebar"><div><span class="chip">${esc(active.lessonId||active.id)} · ${esc(stageTitle(active.stage||state.stage))}</span><h3>${esc(active.title||'Mô phỏng')}</h3><p>${esc(active.purpose||'Đổi biến đầu vào và quan sát đầu ra.')}</p></div><button class="btn primary" data-ai-quick="writing">AI gợi ý kết luận</button></header>
       <section class="mathlab-status">${status.map(x=>`<article><b>${esc(x[0])}</b><span>${esc(x[1])}</span></article>`).join('')}</section>
       ${simCanvaBoard(active,kind)}
       <div class="mathlab-frame-wrap">${src?`<iframe class="mathlab-frame" src="${esc(src)}" loading="lazy"></iframe>`:'<div class="note">Mô phỏng này chưa có file HTML.</div>'}</div>
       <section class="mathlab-panels"><article class="mathlab-formula"><b>Command Window · workflow-cú-pháp</b><pre>${esc(active.formula||'formula = pending')}</pre><p>${esc(active.applicationDomain||active.aiConnection||active.realLifeScenario||'Liên hệ với bài học và lộ trình.')}</p></article><article class="mathlab-output"><b>Output cần đọc</b><ul>${(outputs.length?outputs:effects.length?effects:['Đổi từng biến, giữ biến còn lại cố định, so sánh kết quả trước/sau.']).map(x=>`<li>${esc(typeof x==='string'?x:(x.label||x.name||JSON.stringify(x)))}</li>`).join('')}</ul></article></section>
       <section class="mathlab-vars"><header><b>Input Parameters</b><span>kéo 1 biến → đọc kết quả</span></header><div>${vars.length?vars.map(simRangeHtml).join(''):'<article><p>Chưa có mô tả biến.</p></article>'}</div></section>
       <section class="mathlab-protocol"><h4>Protocol</h4><ol>${steps.length?steps.map(x=>`<li>${esc(x)}</li>`).join(''):'<li>Đổi tham số và ghi kết luận định lượng.</li>'}</ol></section>
     </main>
   </div>
 </div>`;
}
...
function renderWriting(){
 if((A.subjectKind==='mathematics'||A.subjectKind==='programming')) return renderMathSimulationWorkspace();
 ...
 return `<div class="writing-studio step4-writing-studio step36-handwriting-studio">
   ...
   ${mode==='handwriting'?`<div class="writing-workbench step4-writing-workbench step36-writing-workbench handwriting-mode v1285-writing-compact">
     ...
     <aside class="panel writing-control compact-panel slim-control">
       ...
       <div class="handwriting-list clean-scroll">${writeItems.map(w=>`<button class="source-tile hand-tile ${w===activeWrite?'active':''}" data-write-index="${writes.indexOf(w)}"><span>${esc(A.writingTitle?.(w)||w.title||'Nhiệm vụ trình bày')}</span><small>${esc(clip(A.writingPurpose?.(w)||w.purpose||w.prompt_vi||'',86))}</small></button>`).join('')||'<div class="note compact-note">Chưa có nhiệm vụ viết học thuật.</div>'}</div>
     </aside>
     <main class="panel academic-editor compact-panel slim-editor final-academic-editor">
       <div class="toolbar compact-toolbar"><div><span class="chip">${esc(A.writingMode?.(activeWrite)||activeWrite.mode||activeWrite.type||'writing')}</span><h3>${esc(A.writingTitle?.(activeWrite)||activeWrite.title||'Chọn nhiệm vụ viết')}</h3><p>${esc(activeWrite.purpose||'Trình bày ngắn, rõ ý, dùng được trong lớp hoặc môi trường học thuật.')}</p></div><button class="btn primary" data-ai-quick="writing">AI gợi ý viết</button></div>
       ...
       <textarea class="textarea academic-draft compact-draft" data-input="writingDraft" placeholder="Trình bày Programming tại đây...">${esc(state.writingDraft)}</textarea>
     </main>
   </div>`}
 </div>`
}
...
function renderVocab(){
 try{
  const list=getVocab();
  if(!list.length){
   return `<section class="panel vocab-empty learning-recovery-card"><span class="chip">🗂️ Cú pháp</span><h3>Chưa có thẻ workflow-cú-pháp phù hợp</h3><p>Đổi giai đoạn hoặc xóa từ khóa tìm kiếm để xem lại dữ liệu.</p></section>`;
  }
  ...
  const front=`<div class="v1310-flash-face v1312-flash-face"><div class="v1312-flash-visual">${visual}</div><div class="term ${termSizeClass}">${esc(term)}</div><div class="v1310-pron">${esc(info.pron||'↺ Lật thẻ')}</div></div>`;
  ...
  return `<div class="vocab-studio step37-vocab-safe canva3-vocab canva3-vocab-no-hero v1303-vocab-safe v1310-vocab-polish v1311-vocab-luxe v1312-vocab-chibi">
   <div class="vocab-desk step37-vocab-desk canva3-vocab-desk v1303-vocab-desk v1310-vocab-desk">
     ...
     <main class="panel vocab-card-panel canva3-card-panel canva3-card-panel-actions v1303-vocab-card-panel v1310-vocab-main">
       <header class="v1310-vocab-top v1311-vocab-top"><div><span class="chip">Thẻ ${state.vocabIndex+1}/${list.length}</span><h3>${esc(term)}</h3></div><small>${esc(info.pron||'↺ Lật thẻ')}</small></header>
  ...
  return `<section class="panel learning-recovery-card"><span class="chip danger-chip">LỖI TỪ VỰNG</span><h3>Đã chặn lỗi render workflow-cú-pháp</h3><p>${esc(e?.message||e)}</p><button class="btn primary" data-view="overview">Về Tổng quan</button></section>`;
 }
...
function grammarListHtml(xs){
 return xs.map((g,i)=>`<button class="grammar-nav-card ${i===state.grammarIndex?'active':''}" data-grammar-index="${i}"><span>${esc(g.level||'A1')}</span><div><b>${esc(clip(g.title||'Mục workflow-cú-pháp',58))}</b><small>${esc(g.track||'Cú pháp')}</small></div></button>`).join('');
}
function renderGrammar(){
 ...
 return `<div class="grammar-luxe-shell">
   <section class="panel grammar-hero"><div><span class="chip">NGỮ PHÁP RIÊNG · A0 → C1</span><h3>Cú pháp học để nói, viết và đọc tài liệu Bauman</h3><p>Tab này tách workflow-cú-pháp thành lộ trình riêng: chọn cấp nào thì chỉ hiện mạch của cấp đó, tránh lẫn tầng kiến thức và tìm nhanh hơn.</p></div><div class="grammar-hero-stat"><b>${all.length}</b><span>mục workflow-cú-pháp</span></div></section>
   ...
   <main class="panel grammar-main-card"><header class="grammar-main-head"><div><span class="chip">${esc(g.level||'A1')} · ${esc(g.track||'Cú pháp')}</span><h3>${esc(g.title||'Mục workflow-cú-pháp')}</h3><p>${esc(g.why||'Học mục này để nối quy tắc với câu nói, bài viết và tình huống thật.')}</p></div><b class="grammar-index-pill">${xs.length?state.grammarIndex+1:0}/${xs.length}</b></header>
     ...
     <footer class="grammar-footer"><div><b>Chuẩn qua bài</b><span>${esc(g.mastery||'Tự tạo câu mới và dùng được trong vấn đáp thật.')}</span></div>${mapLinks?`<div class="grammar-map-links"><b>Mind map liên quan</b>${mapLinks}</div>`:''}</footer>
   </main></div>
 </div>`;
}
...
function mindMemoryCue(node,idx,parent){
 ...
 if(/workflow-cú-pháp|ký túc|visa|lab|campus|дом/.test(hay))return 'nối cụm → đặt câu';
...
function mediaSmartCategory(m){
 ...
 if(/shadow|phát âm|pronunciation|trọng âm/.test(hay))return 'Cách đọc & Shadowing';
 if(/общежитие|ký túc|lớp|магазин|аптека|sống còn/.test(hay))return 'Case ứng dụng lớp/ký túc';
 ...
 const legacy={'mô phỏng':'Nghe nhập môn','hướng dẫn':'Cách đọc & Shadowing','quy luật':'Mẫu câu nghe-nói','ứng dụng':'Case ứng dụng lớp/ký túc'};
...
function defaultMediaGroups(){return ['Nghe nhập môn','Cách đọc & Shadowing','Case ứng dụng lớp/ký túc','Dự bị/STANKIN','Hội thoại học thuật','Lecture & ghi chép','Email & học thuật','Mô tả dữ liệu','NIR/VKR Bauman','Bảo vệ luận văn','Ôn nghe nâng cao','Video học tập']}
...
function storageGroups(){return [
 ...
 ['Học tập','🎓',['lessons','grammar','exercises','dialogue-bauman-az']],
 ['Cú pháp','🗂️',['vocab','mindmap']],
 ['Cú pháp','🧩',['grammar','grammar-path','mindmap']],
 ['Mind map','🧠',['mindmap','knowledge-index']],
 ['Ứng dụng','🧩',['dialogue-bauman-az']],
 ['Vấn đáp','🎙️',['speaking','deep-speaking-bauman','speaking-link-index']],
 ['Mô phỏng','🔬',['simulations','writing']],
 ['Video/Audio','🎬',['videos']],
...
function sourceLabel(name){return dataSourceMeta(name)?.label||({
 curriculum:'Lộ trình & giai đoạn',lessons:'Bài học',grammar:'Cú pháp gốc','grammar-path':'Cú pháp chuyên sâu',vocab:'Cú pháp',mindmap:'Mind map ôn tập',exercises:'Bài tập',tests:'Kiểm tra',simulations:'Mô phỏng lý thuyết/ứng dụng',speaking:'Vấn đáp giáo sư','dialogue-bauman-az':'Ứng dụng phần mềm vào AI/Bauman','deep-speaking-bauman':'Phản biện ứng dụng sâu','speaking-link-index':'Cầu nối Vấn đáp - Deep',handwriting:'Ký hiệu',writing:'Báo cáo mô phỏng',videos:'Video / Audio','knowledge-index':'Chỉ mục kiến thức'
})[name]||name;}
function sourceIcon(name){return ({curriculum:'🧭',lessons:'📘',grammar:'🧩','grammar-path':'🧬',vocab:'🗃️',mindmap:'🧠',exercises:'📝',tests:'🧪',simulations:'🔬',speaking:'🎙️','dialogue-bauman-az':'🧩','deep-speaking-bauman':'🎙️','speaking-link-index':'🔗',handwriting:'✍️',writing:'📄',videos:'🎬','knowledge-index':'🔎'})[name]||'📦';}
...
function storagePreview(name,data){
 if(isOptionalFile(name)&&!data){
  ...
  return renderOptionalDataGate(name,sourceLabel(name),`${desc||'Nguồn này là dữ liệu lớn cho tab Vấn đáp/Luyện sâu.'}${planned?` Theo kế hoạch có ${planned} mục.`:''} Bấm tải khi cần xem/chỉnh sửa, app không tải sẵn để giữ nhẹ.`);
 }
...
function storageSkeleton(source){
 ...
 const map={
  ...
  grammar:{id,stage,title:'Mẫu workflow-cú-pháp mới',rule:'Quy tắc ngắn gọn',examples:['Пример.'],practice:'Tạo 2 câu dùng mẫu này'},
  'grammar-path':{id,level:'A1',track:'Cách',title:'Mục workflow-cú-pháp mới',why:'Vì sao cần học',core:'Quy tắc lõi',pattern:'Mẫu câu',examples:[{ru:'Пример.',vi:'Ví dụ'}],practice:['Tạo 3 câu'],mistakes:['Lỗi thường gặp'],bauman:'Ứng dụng Bauman'},
  ...
  speaking:{id,stage,group:'custom',context_title_vi:'Case ứng dụng mới',difficulty:'Dễ',difficulty_id:'easy',turns:['Здравствуйте.','Здравствуйте.'],vi_turns:['Xin chào.','Xin chào.'],speakers:['A','B']},
  handwriting:{id,stage,mode:'alphabet',print:'А а',cursive:'А а',note:'Ký hiệu mới',strokes:[{title:'Nét 1',guide:'Quan sát và tô'}]},
  writing:{id,stage,mode:'simulation-report',title:'Báo cáo mô phỏng mới',purpose:'Ghi kết luận sau khi đổi tham số',prompt_vi:'Ghi biến đầu vào, kết quả đầu ra, workflow-cú-pháp giải thích và ứng dụng.',model_ru:'',model_vi:''},
...
function storageGroupFilterTabs(){
 ...
 const activeFiles=(activeMeta[2]||[]).filter(f=>ALL_STORAGE_FILES.includes(f));
 const loaded=activeFiles.filter(f=>!!DB[f]).length;
 return `<div class="storage-group-menu-wrap">
   <details class="storage-group-menu">
     <summary class="storage-group-trigger"><span class="storage-group-ico">🗄️</span><b>Kho môn học</b><u>▾</u></summary>
   ...
   <div class="storage-group-indicator fixed-storage-indicator always-visible" aria-live="polite" title="Nguồn dữ liệu hiện tại"><i>${esc(activeMeta[1])}</i><span>${esc(activeMeta[0])}</span><small>${loaded}/${activeFiles.length||0} nguồn đang có dữ liệu</small></div>
 </div>`;
}
...
function storageAmmunitionShelf(){
 ...
 return `<section class="panel storage-ammo-shelf"><header><div class="storage-shelf-top"><div class="storage-shelf-title"><span class="chip">FILE JSON</span><h3>Nguồn dữ liệu</h3><p>Chọn một nguồn để xem vai trò, đường dẫn và bản ghi.</p></div>${storageGroupFilterTabs()}</div><input class="input" data-input="storageQuery" value="${esc(state.storageQuery||'')}" placeholder="Tìm file, tab dùng, stage, nội dung..."/></header><div class="arsenal-file-scroll">${cards||'<article class="storage-empty"><b>Không có file phù hợp</b><span>Xóa từ khóa hoặc chọn Kho môn học.</span></article>'}</div></section>`;
}
...
function renderStorage(){
 ...
 return `<div class="storage-arsenal storage-vault v1300-storage-arsenal v1314-storage-luxe">
   <section class="panel storage-arsenal-hero"><div><span class="chip">KHO DỮ LIỆU PROGRAMMING JSON</span><h3>Trung tâm dữ liệu Programming Bauman</h3><p>Quản lý nguồn JSON theo nhóm, tab sử dụng và bản ghi. Bố cục mới gọn hơn, dễ tìm, dễ nhập/thay file, dễ xuất và khôi phục.</p></div>${storageQuickStats()}</section>
...
function aiGenerate(mode,prompt=''){
 const c=aiContext();
 if(mode==='lesson')return `<h4>Giải thích bài đang học</h4>${aiBullets([`Giai đoạn: ${c.stage}`,`Bài: ${c.lessonTitle||'Chưa chọn bài'}`,`Ý chính: ${c.lessonSummary||'Chưa có tóm tắt'}`])}<p>Hãy học theo 3 nhịp: đọc ý chính, nói lại bằng tiếng Việt, rồi tạo 2 câu Programming ngắn dùng từ khóa của bài.</p>`;
 if(mode==='vocab')return `<h4>Giải thích workflow-cú-pháp vựng đang chọn</h4>${aiBullets([`Cú pháp/ký hiệu: ${c.vocab.term||'Chưa chọn'}`,`Nghĩa: ${c.vocab.displayMeaning||c.vocab.meaningVi||c.vocab.english||c.vocab.meaningRu||'Chưa có'}`,`English equivalent: ${c.vocab.english||'Chưa có'}`,`Ứng dụng: ${c.vocab.displayApplication||c.vocab.application||'Chưa có'}`])}<p>Mẹo nhớ: đặt từ này vào một câu ở ký túc xá, một câu trong lớp dự bị và một câu trong bối cảnh Bauman.</p>`;
 if(mode==='dialogue')return `<h4>Tạo bài đóng vai nhanh</h4><p><b>Bối cảnh:</b> ${esc(c.dialogueTitle||'vấn đáp học tập')}</p>${aiBullets(['Giáo sư: hỏi bằng câu ngắn, rõ mục đích.','Học viên: trả lời, xác nhận lại thông tin.','Lượt 3: đổi vai và nói nhanh hơn 10%.','Ẩn gợi ý ở vòng cuối để luyện phản xạ giải thích.'])}`;
 if(mode==='review')return `<h4>Ôn tập hôm nay</h4>${aiBullets([`Mục tiêu hôm nay: ${c.goal}`,`Tiếp tục từ tab: ${c.view}`,`Bài hiện tại: ${c.lessonTitle||'chưa chọn'}`,'Ôn 10 từ, 1 vấn đáp, 5 câu ôn tập, 1 đoạn viết ngắn.'])}`;
 if(mode==='writing')return `<h4>Gợi ý mô phỏng</h4>${aiBullets([`Case: ${c.writingTitle||'Chưa chọn mô phỏng'}`,'Ghi 4 phần: biến đầu vào, kết quả đầu ra, workflow-cú-pháp giải thích, ứng dụng AI/Bauman.','Sau khi chạy, tự kiểm tra độ nhạy, ngưỡng bất ổn và lỗi cần tránh.'])}`;
 const p=prompt.trim();
 if(!p)return '<h4>AI Mentor</h4><p>Nhập câu hỏi hoặc chọn một nút gợi ý để AI dùng ngữ cảnh hiện tại của môn học.</p>';
 return `<h4>Gợi ý theo câu hỏi</h4><p><b>Câu hỏi:</b> ${esc(p)}</p>${aiBullets([`Ngữ cảnh: ${c.stage} · ${c.view}`,`Bài hiện tại: ${c.lessonTitle||'chưa chọn'}`,`Mục tiêu hôm nay: ${c.goal}`])}<p>Hướng xử lý: tách bài toán thành giả thiết, biến, workflow-cú-pháp, điều kiện áp dụng và một bài thực hành 5 phút.</p>`;
}
function renderAiMentor(){
 const c=aiContext(); const out=state.aiOutput||aiGenerate('intro','');
 return `<div class="modal-body ai-mentor"><header class="ai-head"><div><span class="chip">AI MENTOR</span><h3>Trợ lý Programming Bauman</h3><p>Dùng ngữ cảnh đang học: bài, workflow-cú-pháp, vấn đáp, ứng dụng, mô phỏng, lịch hôm nay.</p></div><div class="ai-context-card"><b>${esc(c.stage)}</b><span>${esc(c.goal)}</span></div></header><section class="ai-context-grid"><article><b>Bài học</b><span>${esc(c.lessonTitle||'Chưa chọn')}</span></article><article><b>Cú pháp</b><span>${esc(c.vocab.term||'Chưa chọn')}</span></article><article><b>Vấn đáp</b><span>${esc(c.dialogueTitle||'Chưa chọn')}</span></article><article><b>Mô phỏng</b><span>${esc(c.writingTitle||'Chưa chọn')}</span></article></section><div class="ai-quickbar"><button class="btn" data-ai-quick="lesson">Giải thích bài</button><button class="btn" data-ai-quick="vocab">Giải thích workflow-cú-pháp</button><button class="btn" data-ai-quick="dialogue">Gợi ý ứng dụng</button><button class="btn" data-ai-quick="writing">Gợi ý mô phỏng</button><button class="btn primary" data-ai-quick="review">Ôn hôm nay</button></div><div class="ai-chat-grid"><textarea id="aiPrompt" class="textarea ai-prompt" placeholder="Hỏi AI theo bài đang học, ví dụ: giải thích cách dùng cụm này trong lớp dự bị...">${esc(state.aiDraft||'')}</textarea><article class="ai-output">${out}</article></div><div class="modal-actions"><button class="btn" data-act="ai-clear">Xóa kết quả</button><button class="btn primary" data-act="ai-run">Hỏi theo ngữ cảnh</button></div></div>`
}
...
function handleClick(e){
 ...
 if(b.dataset.writing){state.writingMode=b.dataset.writing;save();render();return}
  if(b.dataset.simKind){state.simulationKind=b.dataset.simKind==='application'?'application':'theory';state.simulationTargetId='';state.writingIndex=0;save();render();return}
  if(b.dataset.openSimulation){openMathSimulation(b.dataset.openSimulation,b.dataset.simTarget||'');return}
 ...
 if('writeIndex' in b.dataset){state.writingIndex=Number(b.dataset.writeIndex)||0; if((A.subjectKind==='mathematics'||A.subjectKind==='programming')){const sim=arr(DB.simulations)[state.writingIndex]||{}; state.simulationKind=mathSimKind(sim); state.simulationTargetId=mathSimTarget(sim);} save();render();return}

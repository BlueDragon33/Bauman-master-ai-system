...
(function(){
  ...
  function sessionBlocks(kind, minutes){
    const base = {
      orientation: [
        {key:'video_warmup', label:'Video/mô phỏng ngắn: mở trực giác trước khi vào workflow-cú-pháp', minutes:15},
        {key:'active_listening', label:'Đọc yêu cầu chủ động 2 lượt, khoanh biến/giả thiết chưa rõ', minutes:30},
        {key:'shadowing', label:'Giải lại ví dụ mẫu từng bước', minutes:25},
        {key:'roleplay', label:'Vấn đáp tình huống: định nghĩa, workflow-cú-pháp, lỗi sai', minutes:25},
        {key:'survival_chunks', label:'Cú pháp dùng ngay trong bài tập', minutes:10},
        {key:'symbolic_in_sentence', label:'Ký hiệu phụ trợ qua 1 ví dụ mẫu', minutes:5},
        {key:'reflection', label:'Tự phản hồi lỗi biến đổi/workflow-cú-pháp cần sửa', minutes:10}
      ],
      foundation: [
        {key:'video_context', label:'Xem minh họa workflow lập trình/kỹ thuật', minutes:20},
        ...
        {key:'formula_minimum', label:'Cú pháp tối thiểu để giải được', minutes:10},
        {key:'micro_symbolic', label:'Ký hiệu cực ngắn từ bài vừa giải', minutes:5},
        {key:'write_spoken_sentence', label:'Ghi lại workflow-cú-pháp/bước giải đã dùng', minutes:5},
      ...
      study: [
        ...
        {key:'formula_chunks', label:'Cú pháp phụ trợ dùng ngay', minutes:10},
        {key:'micro_symbolic', label:'Cú pháp qua lỗi/bài mẫu', minutes:5},
        {key:'write_or_type', label:'Ghi lại 3-5 bước giải quan trọng', minutes:10},
        {key:'reflection', label:'Tự phản hồi lỗi workflow-cú-pháp/biến đổi', minutes:5}
      ],
      review_consolidation: [
        ...
        {key:'pronunciation_repair', label:'Sửa workflow-cú-pháp và bước dùng sai', minutes:20},
        {key:'formula_symbolic_light', label:'Cú pháp/ký hiệu phụ trợ', minutes:15},
        {key:'mini_check', label:'Mini-check củng cố', minutes:15}
      ],
      assessment: [
        {key:'listen_warmup', label:'Khởi động workflow-cú-pháp ngắn trước kiểm tra', minutes:10},
  ...
  function routeCards(kind, skill, mission, testLevel, cls={}){
    ...
    if(kind==='orientation') return [
      ...
      {title:'Đọc đề - giải mẫu - trình bày', purpose:'Tạo phản xạ nhận dạng giả thiết, workflow-cú-pháp và bước giải.', limit:'5-8 bước mẫu', route:{view:'learning', learnTab:'practice'}, button:'Luyện giải mẫu'},
      {title:'Vấn đáp tình huống', purpose:'Dùng định nghĩa/workflow-cú-pháp trong bài tập; ký hiệu chỉ hỗ trợ hiểu sâu.', limit:'1-2 vấn đáp', route:{view:'dialogue'}, button:'Vấn đáp'}
    ];
    if(kind==='assessment') return [
      {title:'Khởi động workflow-cú-pháp trước kiểm tra', purpose:'Ôn workflow-cú-pháp ngắn để vào nhịp, không nhảy thẳng vào đề.', limit:'5-10 phút', route:{view:'media'}, button:'Mở ôn nhanh'},
    ...
    if(kind==='review_consolidation'){
      if(cls.blockedTestReason){
        return [
          {title:'Ôn củng cố trước kiểm tra', purpose:'Main báo cuối tuần, nhưng môn chưa đủ điều kiện kiểm tra nên chuyển sang củng cố.', limit:`tối đa ${L.consolidationReviewCards || L.maxVocabCards} workflow-cú-pháp/đơn vị ôn`, route:{view:'vocab'}, button:'Ôn nền'},
          {title:'Bổ sung vật liệu học còn thiếu', purpose:'Hoàn thiện bài học, vấn đáp/thực hành, workflow-cú-pháp hoặc ví dụ còn thiếu.', limit:`${L.maxLessons} bài · ${L.maxDialogues} thực hành`, route:{view:'learning', learnTab:'theory'}, button:'Bổ sung học'},
      ...
      return [
        {title:'Ôn gián đoạn', purpose:'Nhắc lại đúng khoảng cách để nhớ lâu.', limit:`tối đa ${L.maxVocabCards} workflow-cú-pháp`, route:{view:'vocab'}, button:'Ôn workflow-cú-pháp/khái niệm'},
    ...
    return [
      ...
      {title:'Cú pháp/ký hiệu phụ trợ', purpose:'Chỉ chọn workflow-cú-pháp và ký hiệu cần cho bài hôm nay.', limit:`tối đa ${Math.min(L.maxVocabCards||15,15)} workflow-cú-pháp + 1 ví dụ`, route:{view:'vocab'}, button:'Học workflow-cú-pháp'}
    ];
  }
    ...
    const planSessions = sessions.map((s,i)=>{
      ...
      const output = cls.officialTest
        ...
        : (cls.kind==='orientation'
          ? '2 tháng đầu: video/mô phỏng, giải-nhại, shadowing và đóng vai; workflow-cú-pháp/ký hiệu chỉ phụ trợ.'
          : (cls.kind==='review_consolidation' && cls.blockedTestReason
            ...
            : `Hoàn thành phiên giải-trình bày-video trọng tâm, có giới hạn workflow-cú-pháp/ký hiệu và tự phản hồi nhẹ.`));
      return {

...
let state=readState();
let appReady=false;
function defaultState(){return{page:'dashboard',stage:'prepare',module:null,lesson:null,formulaGroup:'all',exerciseDiff:'all',testLevel:'easy',simMode:'observation',activeSim:null,unlocked:{easy:true,medium:false,hard:false,excellent:false},progress:{modules:{},lessons:{},tests:{},simulations:{},simulationPractice:{}},lastScore:null,hostTask:null}}
...
function applyHostTask(task){if(!task||task.subjectId&&task.subjectId!==CONFIG.id)return;state.hostTask=task;if(task.stage&&stages().some(s=>s.id===task.stage))state.stage=task.stage;try{localStorage.setItem(KEY,JSON.stringify(state))}catch{}if(appReady){renderAll();nav(state.page||'dashboard')}}
function connectHostBridge(){const connect=()=>window.BaumanSubjectHost?.onTask?.(applyHostTask);if(window.BaumanSubjectHost)return connect();const script=document.createElement('script');script.src='../shared/host-bridge.js';script.onload=connect;document.head.appendChild(script)}
connectHostBridge();
async function init(){await Promise.all(DATA_FILES.map(loadJSON));const st=stages();if(!st.some(s=>s.id===state.stage))state.stage=st[0]?.id||'prepare';if(!state.module||!modules().some(m=>m.id===state.module))state.module=modules()[0]?.id||'';bind();appReady=true;renderAll();nav(state.page||'dashboard');reportProgress()}
...
function reportProgress(){const o=overall();window.BaumanSubjectHost?.progress?.({stage:state.stage,progress:o.pct,percent:o.pct,total:o.total,done:o.done,completed:o.pct>=100,weakPoints:collectWeak(),recommendedReview:recommendReview(),updatedAt:new Date().toISOString()})}

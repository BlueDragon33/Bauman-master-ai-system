...
function queryTask(){if(params.get('host')!=='main')return null;const querySubject=params.get('subjectId')||subjectId;if(subjectId&&querySubject!==subjectId)return null;const routeLesson=(params.get('routeLesson')||'').slice(0,80),routeView=(params.get('routeView')||'').slice(0,32),routeTab=(params.get('routeTab')||'').slice(0,32);const capabilityRoute=routeLesson&&routeView?{view:routeView,learnTab:routeTab,lessonId:routeLesson}:null;return {type:'BAUMAN_ASSIGN_TASK',protocol:params.get('protocol')||'planning-v3',subjectId:querySubject,courseId:params.get('courseId')||'',taskId:params.get('taskId')||'',missionId:params.get('missionId')||params.get('taskId')||'',stage:params.get('stage')||'',learningItem:params.get('learningItem')||'',durationMinutes:numberParam('durationMinutes'),targetQuestions:numberParam('targetQuestions'),targetScore:numberParam('targetScore'),source:capabilityRoute?'capability-gap-query':'bauman-main-query',...(capabilityRoute?{capabilityRoute,capabilityBand:(params.get('capabilityBand')||'').slice(0,16),capabilityLesson:routeLesson}:{})}}

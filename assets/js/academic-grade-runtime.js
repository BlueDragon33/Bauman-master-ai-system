...
(function(){
  ...
  function form(courseId,code){const def=eventDefinition(courseId,code);if(!def)return '<p>Event không tồn tại.</p>';const state=resultState(courseId,code),r=state.result||{};if(!state.editable)return `<div class="grade14d-locked"><b>${h(state.label)}</b><p>Cần khóa timing bằng nguồn curriculum/assessment có thẩm quyền trước khi cho ghi kết quả.</p></div>`;const confirmed=r.resultConfirmed===true?'checked':'';const source=h(r.resultSource||'');let fields='';if(def.event.gradingNature==='pass_fail'){fields=`<label>Kết quả<select class="field" id="grade14dOutcome"><option value="">Chọn</option><option value="pass" ${r.outcome==='pass'?'selected':''}>зачет / pass</option><option value="fail" ${r.outcome==='fail'?'selected':''}>незачет / fail</option></select></label>`}else{fields=`<label>Numeric score 0–100 (nếu có)<input class="field" id="grade14dScore" type="number" min="0" max="100" value="${r.numericScore??''}"></label><label>Official grade 2–5 (nếu có)<select class="field" id="grade14dGrade"><option value="">Chưa nhập</option>${[5,4,3,2].map(g=>`<option value="${g}" ${Number(r.officialGrade)===g?'selected':''}>${g}</option>`).join('')}</select></label>`}return `<div class="grade14d-form"><div>${badge(state)}</div><label class="grade14d-check"><input id="grade14dConfirmed" type="checkbox" ${confirmed}> Tôi xác nhận đây là kết quả assessment thực tế, không phải rehearsal/ước lượng.</label><label>Nguồn kết quả<input class="field" id="grade14dSource" value="${source}" placeholder="LMS / giảng viên / bảng điểm / ведомость..."></label>${fields}<label>Ghi chú<textarea class="field" id="grade14dNotes" rows="3">${h(r.notes||'')}</textarea></label><div class="grade14d-actions"><button class="btn primary" onclick="saveAcademicGradeResult2026('${h(courseId)}','${h(code)}')">Lưu kết quả</button>${state.result?`<button class="btn" onclick="clearAcademicGradeResult2026('${h(courseId)}','${h(code)}')">Xóa kết quả</button>`:''}</div><p class="academic2026-note">`derivedGrade` nếu có chỉ là ánh xạ từ numeric score theo policy 2024. Hub không tự đánh dấu course COMPLETED và không suy ra dòng phụ lục văn bằng.</p></div>`}

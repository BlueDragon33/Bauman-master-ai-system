...
const STANDALONE_DEVELOPMENT_MODE = (document.querySelector('meta[name="bauman-access-mode"]')?.content || 'standalone').trim().toLowerCase() !== 'managed';
...
const auth={
  current:null,
  setupMode:false,
  init(){if(STANDALONE_DEVELOPMENT_MODE){const stored=getCurrentUser();this.current=stored||{email:'local@bauman.dev',name:'Người học',role:'admin'};$('authScreen').classList.add('hidden');$('appRoot').classList.remove('hidden');this.render();return}const users=readUsers();const stored=getCurrentUser();this.current=stored&&users.some(u=>u.email===stored.email)?stored:null;this.setupMode=users.length===0;this.renderLogin();if(this.current){$('authScreen').classList.add('hidden');$('appRoot').classList.remove('hidden');this.render()}else{localStorage.removeItem(CURRENT_USER_KEY);$('authScreen').classList.remove('hidden');$('appRoot').classList.add('hidden')}} ,
  ...
  logout(){if(STANDALONE_DEVELOPMENT_MODE){toast('Standalone Mode đang bật · không cần đăng xuất');return}localStorage.removeItem(CURRENT_USER_KEY);location.reload()},
...
const mentor={
 open(initial=''){
  ...
  $('aiRoot').innerHTML=`<aside class="ai-panel canva-ai-panel"><div class="ai-head canva-ai-head"><div><h2>✨ Trợ lý AI</h2><small>Tìm môn, lộ trình, lịch và luận văn</small></div><button class="btn sm" onclick="mentor.close()">Đóng</button></div><div class="ai-body canva-ai-body" id="aiLog"><div class="bubble">Bạn muốn tìm môn, giai đoạn hay đề tài НИР nào?</div></div><div class="ai-compose canva-ai-compose"><input id="aiInput" class="field" value="${esc(seed)}" placeholder="Ví dụ: UGV, Python, lịch tuần"><button class="btn primary" onclick="mentor.send()">Gửi</button></div></aside>`;
  $('aiInput').addEventListener('keydown',e=>{if(e.key==='Enter')mentor.send()});
  $('aiInput').focus()
 },
 close(){$('aiRoot').innerHTML=''},
 ask(prompt){const text=String(prompt||'').trim();this.open(text);if(text)this.send()},
 send(){const input=$('aiInput'),q=input?.value.trim();if(!q)return;const res=this.search(q);$('aiLog').innerHTML+=`<div class="bubble user">${esc(q)}</div><div class="bubble">${res}</div>`;input.value='';$('aiLog').scrollTop=$('aiLog').scrollHeight},
 search(q){
  const t=String(q||'').toLowerCase();
  const current=state.subjects[state.subject]||state.subjects[state.lastStudy?.subjectId]||Object.values(state.subjects)[0]||{};
  if((t.includes('giải thích')||t.includes('nội dung đang học'))&&current.id)return `Bạn đang học <b>${esc(current.name)}</b>. ${esc(current.main||current.desc||'Mở môn để xem nội dung chi tiết.')} <button class="btn sm primary" onclick="mentor.close();app.openSubjectInPage('${current.id}')">Mở bài đang học</button>`;
  if((t.includes('ví dụ')||t.includes('ứng dụng'))&&current.id){
   const examples=DATA.courses.filter(c=>c.subject===current.id).map(c=>c.projectUse||c.routeRole||c.deliverable).filter(Boolean).slice(0,2);
   return examples.length?`Ứng dụng gắn với <b>${esc(current.name)}</b>:<br>• ${examples.map(esc).join('<br>• ')} <button class="btn sm primary" onclick="mentor.close();app.openSubjectInPage('${current.id}')">Mở môn</button>`:`Chưa có ví dụ ứng dụng riêng cho <b>${esc(current.name)}</b>. <button class="btn sm primary" onclick="mentor.close();app.openSubjectInPage('${current.id}')">Mở môn</button>`
  }
  if((t.includes('bài tập')||t.includes('hôm nay'))&&current.id){
   const task=findBestLearningTask(current.id);
   if(task)return `Việc gần nhất của <b>${esc(current.name)}</b>: ${esc(task.learningItem||'Học theo lịch')} · ${esc(task.date||todayISO())} · ${Number(task.durationMinutes||90)} phút. <button class="btn sm primary" onclick="mentor.close();app.openSubjectInPage('${current.id}')">Bắt đầu</button>`;
   return `Hôm nay chưa có ca riêng cho <b>${esc(current.name)}</b>. <button class="btn sm primary" onclick="mentor.close();app.page('schedule')">Mở lịch</button>`
  }
  if(t.includes('lộ trình')||t.includes('tiếp theo')){
   const st=roadmapItem(state.roadmapStage||state.schedule?.autoStage||'prepare');
   return `Bạn đang ở <b>${esc(st.title||st.name||'Giai đoạn hiện tại')}</b>: ${esc(st.detail||st.subtitle||st.goal||'Theo lộ trình hiện tại')}. <button class="btn sm primary" onclick="mentor.close();app.page('roadmap')">Mở lộ trình</button>`
  }
  const subj=Object.values(state.subjects).find(s=>(String(s.name||'')+' '+String(s.desc||'')+' '+String(s.main||'')).toLowerCase().includes(t));
  if(subj)return `Tìm thấy trong môn <b>${esc(subj.name)}</b>. <button class="btn sm primary" onclick="mentor.close();pickSubject('${subj.id}');app.page('subjects')">Mở môn</button>`;
  const topic=Object.entries(RESEARCH_TOPICS).find(([id,x])=>(id+' '+x.title+' '+x.short).toLowerCase().includes(t));
  if(topic)return `Nội dung này thuộc <b>${esc(topic[1].title)}</b>. <button class="btn sm primary" onclick="mentor.close();selectResearch('${topic[0]}');app.page('research')">Mở luận văn</button>`;
  if(t.includes('lịch'))return `Mở Lịch học để xem hoặc chỉnh lịch hiện tại. <button class="btn sm primary" onclick="mentor.close();app.page('schedule')">Mở lịch</button>`;
  return 'Chưa tìm thấy kết quả rõ ràng. Hãy thử tên môn, giai đoạn hoặc đề tài như UGV, telemetry, tiếng Nga, Python.'
 }
};
function setHomePanel(mode){state.homePanel=mode;save();app.openHomeFrame(mode)}function showHomeFrame(mode){app.openHomeFrame(mode)}function selectRoadmapStage(id){state.roadmapStage=id;save();app.roadmap()}function pickSubject(id){state.subject=id;state.searchFocusCourseId='';save();app.subjects()}function setSubjectStage(id){state.subjectStage=id;const list=app.filteredSubjectsForStage?app.filteredSubjectsForStage(id):Object.values(state.subjects);if(!list.some(s=>s.id===state.subject))state.subject=list[0]?.id||'russian';save();app.subjects()}function setScheduleView(v){state.schedule.view=v;save();app.schedule()}function changeWeek(n){state.schedule.weekStart=iso(addDays(parseDate(state.schedule.weekStart),n*7));save();app.schedule()}function learningOptionsHTML(subjectId){const courses=app.courseOptionsForSubject(subjectId,state.schedule.autoStage||'prepare');return courses.map(c=>`<option value="${esc(c.id)}">${esc(c.name||c.vi)}</option>`).join('')||'<option value="">Học theo lộ trình</option>'}

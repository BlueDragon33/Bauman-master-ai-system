...
const mentor={
 open(initial=''){$('aiRoot').innerHTML=`<aside class="ai-panel canva-ai-panel"><div class="ai-head canva-ai-head"><div><h2>✨ Trợ lý AI</h2><small>Tìm môn, lộ trình, lịch và luận văn</small></div><button class="btn sm" onclick="mentor.close()">Đóng</button></div><div class="ai-body canva-ai-body" id="aiLog"><div class="bubble">Bạn muốn tìm môn, giai đoạn hay đề tài НИР nào?</div></div><div class="ai-compose canva-ai-compose"><input id="aiInput" class="field" value="${esc(initial)}" placeholder="Ví dụ: UGV, Python, lịch tuần"><button class="btn primary" onclick="mentor.send()">Gửi</button></div></aside>`;$('aiInput').addEventListener('keydown',e=>{if(e.key==='Enter')mentor.send()});$('aiInput').focus()},
 close(){$('aiRoot').innerHTML=''},
 ask(prompt){this.open(String(prompt||''));this.send()},
 send(){const q=$('aiInput')?.value.trim();if(!q)return;const res=this.search(q);$('aiLog').innerHTML+=`<div class="bubble user">${esc(q)}</div><div class="bubble">${res}</div>`;$('aiInput').value='';$('aiLog').scrollTop=$('aiLog').scrollHeight},
 search(q){
  ...
  if((t.includes('ví dụ')||t.includes('ứng dụng'))&&current.id){
   ...
   return examples.length?`Ứng dụng gắn với <b>${esc(current.name)}</b>:<br>• ${examples.map(esc).join('<br>• ')} <button class="btn sm primary" onclick="mentor.close();app.page('subjects')">Xem môn</button>`:`Chưa có ví dụ ứng dụng riêng cho <b>${esc(current.name)}</b>. <button class="btn sm primary" onclick="mentor.close();app.page('subjects')">Xem nội dung môn</button>`;
  }
  ...
  if(t.includes('lộ trình')||t.includes('tiếp theo')){
   ...
   return `Bạn đang ở <b>${esc(st.title)}</b>: ${esc(st.detail||st.subtitle||'Theo lộ trình hiện tại')}. <button class="btn sm primary" onclick="mentor.close();app.page('roadmap')">Mở lộ trình</button>`;
  }
  const subj=Object.values(state.subjects).find(s=>(s.name+' '+s.desc+' '+s.main).toLowerCase().includes(t));

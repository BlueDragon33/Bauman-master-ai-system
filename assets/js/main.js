...
const BRIDGE_TYPES = {ready:'BAUMAN_SUBJECT_READY', task:'BAUMAN_ASSIGN_TASK', progress:'BAUMAN_SUBJECT_PROGRESS', capability:'BAUMAN_SUBJECT_CAPABILITY_STATE', capabilityRequest:'BAUMAN_REQUEST_SUBJECT_CAPABILITY_STATE'};
...
function handleSubjectBridgeMessage(event){if(!trustedSubjectEvent(event))return;const msg=event.data||{};if(!msg||typeof msg!=='object')return;const readyTypes=[BRIDGE_TYPES.ready,'BAUMAN_CHILD_READY'];const progressTypes=[BRIDGE_TYPES.progress,'BAUMAN_PROGRESS_REPORT','SUBJECT_FEEDBACK'];const activeSubjectId=state.activeTask?.subjectId||msg.subjectId||'';if(readyTypes.includes(msg.type)){if(state.activeTask)sendTaskToSubject(event.source,state.activeTask);sendCapabilityRequestToSubject(event.source,activeSubjectId);return}if(msg.type===BRIDGE_TYPES.capability){if(msg.subjectId&&activeSubjectId&&msg.subjectId!==activeSubjectId)return;receiveSubjectCapability({...msg,subjectId:msg.subjectId||activeSubjectId});return}if(progressTypes.includes(msg.type)){if(msg.subjectId&&activeSubjectId&&msg.subjectId!==activeSubjectId)return;receiveSubjectProgress({...msg,subjectId:msg.subjectId||activeSubjectId})}}

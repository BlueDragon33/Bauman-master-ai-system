...
(function(){
  ...
  function blankStore(){return {schema:'bauman_academic_event_readiness_store_v1',version:'PASS14C',users:{}}}
  ...
  function userState(store=readStore()){store.version='PASS14C';store.users=store.users&&typeof store.users==='object'?store.users:{};const scope=currentUserScope();store.users[scope]=store.users[scope]&&typeof store.users[scope]==='object'?store.users[scope]:{events:{}};store.users[scope].events=store.users[scope].events&&typeof store.users[scope].events==='object'?store.users[scope].events:{};return {store,scope,user:store.users[scope]}}
  ...
  function appendPanel(){const root=document.getElementById('page-home');if(!root||root.querySelector('[data-event14c="ledger"]'))return;root.insertAdjacentHTML('beforeend',renderPanel())}

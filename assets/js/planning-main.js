...
(function(){
  ...
  function taskQuery(path,task){const sep=path.includes('?')?'&':'?';const q=new URLSearchParams({host:'main',subjectId:task.subjectId||'',courseId:task.courseId||'',taskId:task.taskId||task.missionId||'',missionId:task.missionId||task.id||'',stage:task.stage||'',learningItem:task.learningItem||task.target||'',durationMinutes:String(task.durationMinutes||''),targetQuestions:String(task.targetQuestions||100),targetScore:String(task.targetScore||80),protocol:'planning-v3'});return path+sep+q.toString();}
  function send(win,task){try{win?.postMessage(task,'*')}catch(err){console.warn('PlanningBridge V3 send failed',err)}}
  ...
  window.addEventListener('message',e=>{const msg=e.data||{};if(!msg||typeof msg!=='object')return;if(msg.type==='BAUMAN_SUBJECT_READY'&&window.state?.activeTask){send(e.source,window.state.activeTask)}if(msg.type==='BAUMAN_SUBJECT_WARNING')storeWarning(msg);if(msg.type==='BAUMAN_SUBJECT_SCHEDULE_REQUEST')storeWarning({...msg,status:'warning',message:msg.reason||'Môn học yêu cầu Main điều chỉnh lịch.'});if(['BAUMAN_SUBJECT_PLAN','BAUMAN_SUBJECT_ROUTE_PLAN','BAUMAN_SUBJECT_INTERNAL_PLAN'].includes(msg.type))storePlan(msg);if(msg.plan&&msg.plan.sessions)storePlan(msg);});

...
import { loadRoadmapPriority } from "./priority.mjs";
...
function resolvePinned(baseDir, descriptor, id) {
  assert(typeof descriptor?.path === "string" && !path.isAbsolute(descriptor.path), `Invalid Scheduler path for ${id}`);
  const file = path.resolve(baseDir, descriptor.path);
  assert(file.startsWith(`${baseDir}${path.sep}`), `Scheduler path escapes package: ${descriptor.path}`);
  assert(fs.existsSync(file), `Missing Roadmap Scheduler file: ${descriptor.path}`);
  const bytes = fs.readFileSync(file);
  assert(bytes.length === descriptor.bytes, `Roadmap Scheduler byte count mismatch: ${descriptor.path}`);
  assert(sha256(bytes) === descriptor.sha256, `Roadmap Scheduler hash mismatch: ${descriptor.path}`);
  return { file, bytes };
}
...
export function loadRoadmapScheduler(options = {}) {
  const baseDir = path.resolve(options.baseDir || defaultBaseDir);
  const manifest = readJson(path.join(baseDir, "scheduler", "manifest.json")).value;
  assert(manifest.schema === "BAUMAN_ROADMAP_V2_SCHEDULER_MANIFEST_V1", "Unsupported Roadmap Scheduler manifest schema");
  assert(manifest.status === "PASS_B103_DETERMINISTIC_WEEKLY_PROJECTION_HARNESS", "Roadmap Scheduler manifest is not B103 PASS");
  assert(manifest.mode === "in_memory_weekly_projection_harness", "Roadmap Scheduler is not an in-memory projection harness");
  assert(manifest.productionIntegration === "disconnected", "Roadmap Scheduler production boundary is not locked");

  resolvePinned(baseDir, manifest.upstream.priorityManifest, "priorityManifest");
  const contract = pinnedJson(baseDir, manifest.files.schedulerContract, "schedulerContract");
  const contractSchema = pinnedJson(baseDir, manifest.files.schedulerContractSchema, "schedulerContractSchema");
  const requestSchema = pinnedJson(baseDir, manifest.files.schedulerRequestSchema, "schedulerRequestSchema");
  const resultSchema = pinnedJson(baseDir, manifest.files.schedulerResultSchema, "schedulerResultSchema");
  resolvePinned(baseDir, manifest.files.schedulerEngine, "schedulerEngine");
  const priority = loadRoadmapPriority({ baseDir });
  ...
  assert(contract.mode.calendarWriteEnabled === false && contract.mode.persistentStoreEnabled === false && contract.mode.runtimeWriteAllowed === false, "Scheduler enables unsafe writes");
  ...
  const validateRequest = (input) => {
    ...
    assert(input.schema === requestSchema.$id, "Scheduler request schema mismatch");
    ...
    for (const item of input.items) {
      ...
      assert(!Object.hasOwn(item, "priorityResult"), "Caller-supplied Priority result is forbidden");
      const candidate = priority.validateCandidate(item.priorityCandidate);
      assert(candidate.phaseId === input.phaseId, `Scheduler item/Priority phase mismatch: ${item.scheduleItemId}`);
      ...
      assert(item.source.verified === true, `Unverified Scheduler source: ${item.scheduleItemId}`);
      ...
      if (currentActivity) assert(item.source.kind === contract.sourcePolicy.currentBaumanSourceKind, `Current-subject activity lacks official source: ${item.scheduleItemId}`);
      if (item.activityKind === "nir_thesis") assert(item.source.kind === contract.sourcePolicy.nirSourceKind, `NIR activity lacks verified plan: ${item.scheduleItemId}`);
      if (["current_bauman_official", "nir_plan_verified"].includes(item.source.kind)) assert(input.phaseId === "GD3", `Dynamic Master Mode source outside GD3: ${item.scheduleItemId}`);
      ...
      if (!technical) assert(item.russianTwinMinutes === null, `Non-technical item has Russian twin budget: ${item.scheduleItemId}`);
    }
  ...
  const projectWeek = (input) => {
    ...
    for (const item of request.items) {
      ...
      if (reason) {
        deferred.push({ scheduleItemId: item.scheduleItemId, targetId: priorityResult.targetId, reason, priorityRank: priorityResult.rank });
      } else {
        eligible.push({ item, priorityResult });
      }
    }
    ...
    const unscheduledCriticalTargets = deferred
      .filter((entry) => rankByCandidate.get(request.items.find((item) => item.scheduleItemId === entry.scheduleItemId)?.priorityCandidate.candidateId)?.criticalOverride)
